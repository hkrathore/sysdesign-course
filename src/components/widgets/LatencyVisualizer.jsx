  ...
  return (
    <div className="not-content w-full max-w-3xl mx-auto font-mono text-[var(--w-text)]" style={{ background: "transparent" }}>
      <div className="rounded-xl border border-[var(--w-border)] p-5 shadow-2xl" style={{ background: "var(--w-bg)" }}>
        ...
        {/* legend */}
        <div className="flex flex-wrap items-stretch gap-3 mb-4 text-[11px]">
          {Object.values(TIERS).map((t) => (
            <span key={t.label} className="flex items-center gap-1.5 text-[var(--w-muted)]">
          ...
          {DATA.map((d) => {
            ...
            return (
              <div key={d.name} className="flex items-center gap-3">
                <div className="w-48 shrink-0 text-[11px] text-[var(--w-text)] text-right leading-tight">{d.name}</div>
                <div className="flex-1 h-6 rounded overflow-hidden relative" style={{ background: "var(--w-panel)" }}>
                  <div
...
function Reflex({ c, t, s }) {
  return (
    <div className="h-full flex flex-col rounded-lg border border-[var(--w-border-soft)] p-2.5" style={{ background: "var(--w-panel)" }}>
