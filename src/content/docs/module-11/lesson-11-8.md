...
Two facts decide everything downstream. First, **output tokens are typically priced 3–5× higher than input tokens** (the decode phase, 11.1, runs the full model once per emitted token; prefill batches the input). So a 200-token answer can cost more than a 2,000-token prompt. **Output is the lever you most directly control** — cap it, structure it, stop padding it. Second, **long prompts inflate the input cost on *every single call*.** Chat history and RAG context (11.3) are re-sent each turn because the model is stateless (11.1); a 20k-token retrieved context isn't a one-time cost, it's a tax on every message in the session. The Director-altitude statement: *you are not optimizing a model, you are optimizing tokens — and the two biggest token sinks are output you didn't cap and context you re-send.*
...
- **Trim and compress context.** Don't re-send the entire chat history every turn; window it, or summarize older turns into a compact running summary (11.11). For RAG, **retrieve fewer, better chunks** — a reranker (11.3) lets you send the top 3 instead of the top 20, cutting input tokens *and* improving quality (less "lost in the middle").
...
| **Quantization** (self-host) | memory + faster inference | small measured accuracy loss; self-host only | you control serving (11.4) and can tolerate the precision cost |
