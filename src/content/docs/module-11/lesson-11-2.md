...
**(e) Triggers.** A background sync fires on **reconnect** (connectivity regained), **foreground** (user opened the app), **periodic** (every N minutes, coalesced by the OS scheduler), and **push-driven** (an APNs/FCM ping or a live socket says "you have changes"). Push-driven is the efficient default; blind periodic polling is the battery killer in part 6.
