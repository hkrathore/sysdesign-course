...
> Offline-first inverts the client: the **local store is the source of truth** and the network is a background enhancement, so airplane mode is "sync paused," not an error. The design turns on three mechanisms: a **cursor** so each sync pulls the delta, not the account (a 250x saving on a 10 MB store), **tombstones** so deletes propagate instead of resurrecting, and an **idempotent replay queue** because delivery is at-least-once. Get the cadence wrong and a 60-second poll wakes the radio ~1,300 times a day for nothing.
