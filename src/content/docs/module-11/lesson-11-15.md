...
The answer shape is **Clarify → Options → Decide-and-Sequence → Name the trade-off and reversibility.** It's the hypothetical-decision structure (Lesson 10.2) applied to strategy.
...
- **Options** — lay out the spectrum honestly, cheapest-and-fastest to most-expensive-and-slowest:
  ...
  - **Fine-tune / adapt** (LoRA on an open model, or a vendor's tuning API): narrow behavior/format/latency wins. Costs: data curation + an eval harness + retraining as base models move (Lesson 11.5).
...
- **Decide and sequence** — the answer is rarely one option; it's an order. The default sequence: **(1) Buy** — ship on a frontier API (with RAG, Lesson 11.3) behind a gateway to learn what customers value, fast. **(2) Build the moat** — invest in proprietary data, an evaluation harness, and product/UX, the things that *don't* commoditize. **(3) Self-host or fine-tune** — only the workloads where sustained volume, data residency, or control have earned it.
- **Name the trade-off and reversibility.** Buying on an API is **reversible** (a gateway lets you swap providers — Lesson 12.3). Standing up an inference platform and team — or training your own model — is a **one-way door**. Spend conviction in proportion: experiment freely behind reversible doors; demand overwhelming evidence before walking through a one-way one.
...
- **Names the trade-off it's accepting and ties it to reversibility** — the Lesson 10.1 rule applied to strategy: the cost is stated, and conviction is spent in proportion to how hard the door is to reopen.
...
- **Puts measurement first** (cost-per-request + eval bar in Q1) — ties strategy to the unit economics and governance a Director owns (Lesson 11.16).
...
- **"How do you avoid betting the company on one vendor?"** — *Strong:* a model gateway/abstraction layer (Lesson 12.3) that makes providers swappable and routes by cost/quality, plus a fallback provider for availability — optionality by design. *Red flag:* "we'd self-host to be safe" — trades a reversible dependency for a far larger fixed cost and slower capability.
...
> **Spaced-repetition recap:** The AI strategy round scores **altitude, currency, and judgment under hype**, not which model you'd name. Answer in **Clarify → Options → Sequence → Trade-off/reversibility**: clarify data-sensitivity, scale, control/customization, speed-to-market, team, lock-in; lay out API → self-host → fine-tune → (rejected) build-from-scratch; commit to the sequence **buy-to-learn → build the moat → self-host where earned**; name the trade and whether the door is reversible. The 2026 calibration vs the 2023 hype: **the model is a commodity input, not the moat** — own the **data, eval, distribution, and product**, design optionality with a **gateway**, self-host only past the **utilization crossover** or under a **residency** constraint, and never propose training a foundation model from scratch as an application company. Cross-ref: 8.6 (build-vs-buy), 11.4 (self-host / serving economics), 11.5 (fine-tune), 11.16 (governance, risk & cost), 12.3 (the gateway that makes it all reversible).

---

*End of Lesson 11.15. Strategy decides what you build on; Lesson 11.16 covers what you're accountable for once it's live — the governance, risk, data-privacy, and cost discipline a Director owns when nondeterministic systems touch real customers and a real budget.*
