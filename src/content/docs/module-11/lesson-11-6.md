...
### Learning objectives
- Choose a **rendering strategy** (CSR, SSR, SSG, ISR, streaming SSR + React Server Components, edge) from the requirement, content dynamism, SEO need, TTFB and interactivity, and name what each choice rejects.
...
The decision rule a Director states out loud: pick by **content dynamism** (static → SSG/ISR, per-request dynamic → SSR/streaming, behind-auth app → CSR), **SEO need** (public and discoverable → server-render, behind login → CSR is fine), **TTFB and first-paint budget**, and **interactivity** (heavy app → accept hydration, mostly-read → ship less JS). "SPA everything" is the anti-pattern, it silently rejects SEO and first paint for the exact pages that drive organic acquisition.
