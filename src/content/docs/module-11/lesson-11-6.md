...
> Frontend architecture at scale is two decisions wearing one name: **where and when the HTML gets built** (client, server, build time, or edge, chosen per surface, not per fashion) and **how many teams can ship without colliding** (micro-frontends are microservices' org trade, distributed-monolith risk included). The scoreboard is public: Core Web Vitals (LCP under 2.5 s, INP under 200 ms, CLS under 0.1) feed search ranking and conversion, so they are a release gate, not a vibe. "SPA everything" is the answer that fails both.
