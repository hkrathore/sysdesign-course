...
3. **Rate-limit + quota check** against Redis. Over the req/s limit → `429`; over the spend budget → `402`/`429` with a clear reason (the switchboard refusing the over-budget team). Fail-fast, microseconds.
4. **Cache lookup**: exact key in Redis; if miss and the route opts into semantic caching, embed + ANN (approximate nearest neighbor) search the vector store within the similarity threshold. Hit → return immediately (single-digit ms, no provider call, no token cost), the switchboard answering without placing the call.
