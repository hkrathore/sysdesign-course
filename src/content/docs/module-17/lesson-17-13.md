...
```mermaid
flowchart TD
    Q["Hire-a-leader question"] --> BAR["Bar<br/>judgment and teams-built evidence and hard people calls, hire better than you"]
    BAR --> LOOP["Loop<br/>deep behavioral and live working session and cross-functional panel"]
    LOOP --> SIG["Signal weighting<br/>references highest, working session next, interview lowest"]
    SIG --> REF["References<br/>on-list and back-channel, a report a peer a boss"]
    REF --> RATIO["Internal / external ratio<br/>build the bench, name the cost of each extreme"]
    RATIO --> STOP["Stop and let them probe"]
    STOP --> D1["A leader hire that failed"]
    STOP --> D2["Onboarding a senior leader, 30/60/90"]
    STOP --> D3["Internal promotion vs external gap"]
```
...
- **Every leadership req now has to justify itself.** Flatter orgs, fewer manager slots, and a player-coach expectation mean a new leadership head is a budget line you defend, not a default of growth. "I hire great leaders whenever I find them" reads as ZIRP-era (ZIRP = zero interest-rate policy) empire-building. The credible opener asks whether the leadership work can be absorbed by an internal promotion, a wider span, or fewer-larger teams *before* it opens an external req.
