---
title: "17.3 - The Story Portfolio"
...
1. **A real, dated event you owned**, your decision, not your team's or your VP's. The probe is built to find the seam between what you did and what you're claiming.
2. **A quantified spine**, team size, dollars, percentage, timeline, a before/after metric. A story with no number is the same red flag "it scales" is in a system-design round (the house rule).
...
| 4 | **A layoff or hard constraint you owned** | You own decisions made above you and rebuild after | "Finance gave me a list", distancing, is the signature efficiency-era fail |
...
| 6 | **An incident you COMMANDED** | You coordinate under fire, you don't type the fix, Director, not senior IC (individual contributor) | Hands-on-fixer-with-no-command-structure reads a level too low |
...
- **Down the columns**, every category needs at least one strong story, ideally two, so two interviewers comparing notes don't hear the same one. A column with zero strong entries is a gap you fix *before* the loop (a closet hole found before you pack).
...
The portfolio's efficiency comes from one story serving multiple columns. Here's a database migration told three ways, each pulling a different thread for a different category. **Same facts, same numbers, different load-bearing decision**, the muscle to build is not three stories but one you can rotate (one garment, three outfits).
...
> **Told for Execution, delivery under constraint:**
...
> **Told for Influence, cross-org without authority:**
...
> **Told for the wrong-call slot, a decision you got wrong:**
...
What a portfolio must contain shifted with the environment (the six shifts are this track's spine):

- **The growth-era story is now a liability if it's all you have.** "I scaled my org from 10 to 60" was a 2015 flex; in 2026, with no efficiency angle, it codes as ZIRP (zero interest-rate policy) empire-building. The refresh: pair it with output-per-dollar, or lead with a *more-with-less* story. A bank stale since the growth era is the most common dated-prep tell.
