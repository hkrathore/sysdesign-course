---
title: "17.10 - Influence, Disagreement & Executive Communication"
...
- **Steel-man the exec's position.** State their case fairly, and *generously*, name what made it the reasonable call from where they sat. This is the trust deposit (the diplomat stating his position better than he did); skip it and the rest reads as a grievance. It also proves you understood the decision before you opposed it.
...
- **The forum and the time-box.** Challenge in private, the 1:1 or staff meeting, not the all-hands, and make the case *once*, hard, with a clear escalation path if it matters enough. Unified front in public (the diplomat never leaks her dissent). Relitigating in every meeting is the fail; so is never raising it.
- **Resolution, either way.** *Won* → show their concern was still addressed, so it wasn't a turf victory. *Lost* → a **visible commit with personal cost** (best people staffed on it, presented to your org in your own voice as the plan, never relitigated) **plus a tripwire**, the metric or date at which it gets revisited. The tripwire is the senior marker (the diplomat's quietly noted indicator): it lets you commit fully *without* pretending you were wrong.
...
```mermaid
flowchart TD
    D["Disagreement up chart"] --> SM["Steel-man their position<br/>state it generously"]
    SM --> CASE["Data-backed case<br/>concrete costed alternative"]
    CASE --> FORUM["Right forum and time-box<br/>private once unified in public"]
    FORUM --> OUT{"Outcome"}
    OUT -->|Won| ADDR["Show their concern<br/>was still addressed"]
    OUT -->|Lost| COMMIT["Visible commit with cost<br/>staff it carry it down in your voice"]
    COMMIT --> TRIP["Set a tripwire<br/>metric or date to revisit"]
    TRIP --> UPDATE["Honest was-I-right<br/>name what they saw"]
    ADDR --> UPDATE
```
...
- **Headcount questions now run in reverse.** "Tell me about *losing* a headcount fight" and "where does AI change your headcount math?" A pure "I won the ask, I got my five engineers" story reads as unaware post-ZIRP (zero interest-rate policy); the strong version starts from "headcount is the expensive option" and considers leverage first.
...
- **Cross-org stories are now migration-, platform-, or AI-adoption-shaped, and you supply the mechanism.** No PMO to delegate the coordination to. The Director personally supplies it: a single DRI, a thin-slice proof that de-risks the ask, a shared OKR (objectives and key results), and escalation framed as *one crisp decision request*, not a complaint about another team.
