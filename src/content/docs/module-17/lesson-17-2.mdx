---
title: "17.2 - The Four Answer Frameworks"
...
**Why it scores:**
- Position rejects the false binary up front ("not a fixed identity"), the post-founder-mode calibration.
...
- **Numbers moved from nice-to-have to mandatory.** 2025-26 rubrics explicitly demand quantification in behavioral answers, a STAR with no figure in the Result now scores as a red flag, the same way "it scales" fails a system-design round (the cross-track house rule).
- **The hypothetical got an efficiency-era twist.** "Scale 15 → 50" used to mean "draw the org chart." Now the first clarify is *whether you hire at all*, headcount is the expensive option, and a candidate who reaches straight for bodies reads as a ZIRP (zero interest-rate policy) relic.
- **Philosophy answers must sound current.** "Servant leader, hire great people and get out of the way" was a fine 2015 Position; in 2026 it's near-disqualifying in front of founders (post-founder-mode), and psychological safety stated as *comfort* rather than Edmondson's risk-taking-plus-accountability is suspect.
...
- **Stories with no number and no rejected alternative.** Both are house-rule violations, an unquantified result and an option-less "decision" each fail the same way "it scales" fails system design.
...
*End of Lesson 17.2. The four frameworks are your tray of instruments; the next lesson builds the material they operate on, a 12-15 story portfolio in a coverage matrix, each story quantified and probe-tested to L3, mapped so one story can serve two or three categories. These shapes then apply category by category, starting with leadership philosophy.*
