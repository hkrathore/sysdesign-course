---
title: "13.4 - Privacy Engineering & Data Deletion"
...
> "Delete this user" is not a query, it is a manhunt: the person has fanned out into replicas, caches, search indexes, the warehouse, queue backlogs, logs, backups, and third-party processors, and the law gives you **30 days** plus a proof obligation. The design turns on one split: **hard-delete** the stores you can edit, **crypto-shred** the immutable ones (encrypt per user, destroy the key, and every backed-up copy turns to noise). The cheapest control sits upstream: a field never collected is one you never inventory, secure, or delete.
...
- Treat the **30-day DSAR (data subject access request) SLA (service-level agreement) and the audit trail** as first-class requirements: you must not only delete, you must *prove* you deleted, across every store, within the window, with a record that survives the user being gone.
...
- **Primary DB**, the obvious one (the front-desk row), a hard `DELETE` or anonymizing update on the user row and their child records.
- **Read replicas**, deletion propagates through normal replication, but you verify it landed rather than assume it.
- **Caches** (Redis, CDN, app-tier), the user's cached profile, session, and any keyed response must be evicted or it serves stale-but-deleted data for the TTL (time-to-live).
...
- **Third-party processors**, every SaaS you forwarded the data to (Stripe, Segment, your email provider, analytics), each needs a deletion call or contractual deletion; their copy is your responsibility under the law (the loyalty partner's mailing list).
...
*End of Lesson 13.4. Privacy is a fan-out with a deadline and a receipt: enumerate every store, crypto-shred the immutable ones, collect less than you can, and prove you finished.*
