---
title: "13.1 - Security & Trust for System Designers"
...
> Security is the **floor plan**, not the lock bolted on the night before launch: a system property designed in at the **trust boundaries**, where a request crosses into a more privileged zone. Everything turns on one discipline: threat-model those crossings with **STRIDE**, triage to the top two or three, and buy risk reduction at a named price in latency, friction, or velocity. The anchor numbers: MFA cuts account takeover by **over 99%**, an average breach runs **\$4–5M**, so the heaviest locks go where the loss is largest.
...
- **Latency.** A WAF (web application firewall) and per-hop mTLS (mutual TLS) add real milliseconds; a synchronous fraud check before a login adds 50–200 ms. You put heavy checks only where the risk justifies the latency (the heaviest locks go on the vault).
- **Friction.** Mandatory MFA (multi-factor authentication) cuts account-takeover by **over 99%** but adds a step to every login and a support cost for locked-out users; you mandate it for admins and high-value accounts, and risk-base it for everyone else.
...
*End of Lesson 13.1. This is the mental model the whole Security & Trust track rests on: security is a system property designed in from trust boundaries, paved-road defaults, and assume-breach blast-radius thinking, reasoned in CIA, STRIDE, and zero-trust before any control.*
