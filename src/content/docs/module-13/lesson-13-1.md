...
- **Latency.** A WAF (web application firewall) and per-hop mTLS (mutual TLS) add real milliseconds; a synchronous fraud check before a login adds 50–200 ms. You put heavy checks only where the risk justifies the latency.
