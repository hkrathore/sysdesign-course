...
3. **Random disk I/O (~10 ms HDD) is catastrophic on the hot path → batch into sequential I/O.** This is *why* LSM-trees (LSM = log-structured merge) exist (turn random writes into sequential), and why the SSD era reshaped database design.
