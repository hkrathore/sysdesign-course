...
> The latency ladder is a map of distances inside the computer: RAM is a ~100 ns walk down the hall, a same-datacenter round trip is ~5,000× farther, and a cross-continent call is ~150 ms, a million-fold beyond memory. Interviewers score the ratios, not the digits, because the ratios dictate the reflexes: cache aggressively, keep synchronous cross-region calls off the hot path, batch random I/O into sequential. The senior twist is tail amplification: fan out to 100 dependencies, each 1% slow, and ~63% of requests wait on a straggler.
...
1. **Memory is ~5,000× faster than a network round trip → cache aggressively and minimize hops.** Every avoided same-DC round trip is worth ~5,000 memory reads (a hallway walk instead of a flight).
2. **Cross-region is ~150 ms → never put a synchronous cross-region call on the request path.** Replicate data near users; do cross-region work async. One synchronous transatlantic hop blows most user-facing p99 budgets by itself (a trip around the world per request).
3. **Random disk I/O (~10 ms HDD) is catastrophic on the hot path → batch into sequential I/O.** (One drive across town, not twenty.) This is *why* LSM-trees (LSM = log-structured merge) exist (turn random writes into sequential), and why the SSD era reshaped database design.
