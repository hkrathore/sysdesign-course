...
> Estimation is a feasibility sniff test: three minutes of arithmetic whose only deliverable is one sentence, does this fit on 10 servers or 10,000? Four numbers in a fixed order: QPS from DAU over ~10⁵ seconds a day, storage from writes (never reads) times payload times replication, bandwidth, then servers from peak QPS over per-server capacity. Round aggressively and state every assumption: off by 2× is fine, off by 100× changes the architecture, and computing 4,629.6 QPS when "~5k" will do is itself an anti-signal.

### Learning objectives
- Produce QPS (queries per second), storage, bandwidth, and server-count estimates in under three minutes.
...
4. **Servers.**
   `servers ≈ peak QPS ÷ per-server capacity`. This is the number that tells you whether you're building a small fleet or a hyperscale system (the sniff test's verdict).

**Worked numeric example, a Twitter-scale write path (illustrative; assumptions stated):**
- Assume 200M DAU (daily active users), each posts 2 tweets/day → 400M tweets/day.
...
The Director-level point isn't the digits, it's the conclusion: *reads dominate by 100×, so the architecture lives or dies on the read/cache path, and text storage is trivial while media is the real cost.* The math exists to justify that sentence (the engineer's eyeball pass, not the detailed calc).
...
> *Model:* 500M × 40 = 20B msgs/day ÷ 86,400 ≈ **~230k/s** avg. Peak ×3 ≈ **~700k/s**. Conclusion: write-heavy, so a partitioned log/queue front door and LSM-tree (LSM = log-structured merge) storage, not a read-cache architecture.
