...
The course is organized into **six Parts** (18 modules):
...
| **III · Specialized Design Tracks** | 6–12 | LLD/OOD, data-platform, Gen AI & Agentic, client & mobile, and multi-tenancy & SaaS tracks |
| **IV · Engineering Excellence & Operations** | 13–15 | Security & trust, testing & quality, production troubleshooting |
| **V · Strategy & Leadership** | 16–17 | Architecture-and-org strategy, and the behavioral half of the loop |
| **VI · Capstone** | 18 | A fresh problem you drive, plus the red-flags/strong-signals rubric |

> **All six Parts are complete** — 18 modules end to end, including the Client & Mobile and Multi-Tenancy & SaaS design tracks in Part III, authored at the same Director altitude, with the same quantify-and-name-the-trade-off discipline, as the rest of the course.
...
- **Probe simulator** (three-levels-deep follow-up drill per behavioral question), *Module 17*
- **Story-coverage matrix** (leadership stories × categories; gap highlighting + the mandatory story slots), *Module 17*
...
**The expanded problem set, pick by the loop you're walking into.** These are the questions the canonical Module 4 set leaves out: **Module 6** the LLD/OOD curveballs (parking lot, elevator, Splitwise, restraint and clean modeling, not infra depth), **Module 16** the architecture-and-org-strategy round that is *most* Director-distinctive (monolith breakup, migrations, build-vs-buy, Conway org design), **Module 5** the non-canonical business-domain HLDs (payments, auctions, Google Docs, "design Kafka"). Don't read all of them, let the company tell you: Amazon/Adobe lean LLD (Module 6), Director/Head-of-Eng loops lean strategy (Module 16), fintech/marketplace targets lean Module 5. Each module's cheat sheet carries the crux of every problem in ~5 minutes; deep-read only the 3-4 closest to your target.

**Module 17, the leadership track is not optional.** ~40% of a Director loop is leadership, and it's where most strong system designers lose the offer. Read **17.1-17.3** (the calibration, the four answer frameworks, the story portfolio) first, they're the spine, then the category lessons that match your gaps. Use the **probe simulator** (17.2) and **story-coverage matrix** (17.3) as drill tools, and build your real story bank early; a quantified, probe-resistant portfolio is the single highest-ROI prep on this whole site.
...
**The Engineering Excellence track (Modules 13–15) is the operational half of the Director remit.** Two kinds of question live here, and both are now standard: a security/quality *design* round ("design auth for this," "make this PCI-compliant," "design our CI/CD and test platform") and an operational-*leadership* probe ("latency just doubled, walk me through how you'd investigate," "how do you run on-call and incidents for a large org?"). The signal is whether you own these as **systems with paved-road defaults and guardrails**, not checklists bolted on at the end, and whether you know where the decision turns on depth versus where you delegate with a stated prior. **Module 13** is security, privacy & trust: the threat-model/zero-trust lens, auth at scale, secrets/KMS and envelope encryption, privacy and right-to-be-forgotten as an *architectural* constraint, compliance-as-architecture, and abuse/fraud/DDoS. **Module 14** is testing & quality as an *operating system*: the test pyramid and where the ROI actually is, contract testing, ephemeral environments, flaky-test economics, shift-left vs test-in-prod, and quality gates/DORA. **Module 15** is production troubleshooting: the diagnostic method, observability-in-anger, SLOs and error budgets, incident command, blameless postmortems, and graceful degradation/on-call health. Read the **13.1, 14.1, and 15.1** framing lessons as the spine, then the topics your loop will probe, fintech and regulated targets lean Module 13, platform and infra roles lean Modules 14–15. The two non-negotiables hold here too: quantify (latency, blast radius, error budget, the flake math), and name the trade-off and the rejected alternative.

**The Client & Mobile track (Module 11) is the half of the system the user actually holds.** Most of this course is server-side; but for consumer and product companies the client is where perceived latency, offline capability, and retention are won or lost. **Module 11** treats the client as a first-class, frequently-partitioned node: the thin-vs-thick placement decision and the five constraints the server never has (intermittent network, device and battery budgets, app-store release latency, heterogeneous versions, cold start), offline-first and local-first data sync, conflict resolution (last-write-wins vs OT vs CRDT vs server-authoritative), mobile push and real-time delivery at scale, client performance and resource budgets, and web/frontend architecture (rendering strategies, Core Web Vitals, BFF, micro-frontends), capped by a full offline-first mobile design problem. Read **11.1** as the spine, then the topics your loop leans on. The two non-negotiables hold here too: quantify (latency, battery, sync payloads), and name the trade-off and the rejected alternative.

**The Multi-Tenancy & SaaS track (Module 12) is the architecture behind every B2B product.** Every business design silently assumes a single tenant; the multi-tenant problem, serving many customers on shared infrastructure without leaking data across them or letting one starve the rest, is a distinct design round in every SaaS Director loop. **Module 12** runs the isolation spectrum (silo, bridge, pool) as its spine: tenant isolation models (database-per-tenant vs schema-per-tenant vs shared-schema with row-level security, and cross-tenant data leakage as the cardinal failure), per-tenant limits, metering and billing with noisy-neighbor control, and the control-plane/data-plane split with the full tenant lifecycle (provisioning, residency, deletion), capped by a full multi-tenant SaaS platform design problem. Read **12.1** as the spine, then the rest. Quantify (density per host, cost per tenant, blast radius), and name the trade-off and the rejected alternative.
