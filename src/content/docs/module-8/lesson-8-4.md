...
**Assumptions:** **~50M MAU** (monthly active users) at **~40 events/day** → `≈ 2B events/day`, sustained `≈ 23k events/sec`, peak ~3× → **~70k events/sec** (a 200M-MAU product lands at ~200k/sec peak, ~20B/day). Each raw event ~**500 bytes** (event name, identity, timestamp, flexible property bag).
...
- *Choice:* **Kafka** (partitioned, durable, replayable), the 14.3 ingestion substrate, tee'd to both consumers (the one wiretap feeding both) and partitioned by a **stable identity key** so one user's events land in order on one partition, which is what makes in-stream sessionization and ordered-funnel correctness possible.
...
- *Choice:* a **columnar real-time OLAP engine** (Druid / ClickHouse / Pinot, the 14.2 store), fed from the stream, events laid out **event-time-ordered and partitioned by day** with the common funnels **pre-aggregated**, serving sub-second funnel/retention/cohort/segmentation over the bounded grammar (the front desk's fast, pre-arranged index).
- *Rejected, serve the interactive surface from the lakehouse:* a cold scan over billions of events is seconds-to-minutes, not the sub-second the product promises; the lakehouse is flexible, not interactive. *Rejected, an OLTP (online transaction processing) row store:* high-cardinality per-step funnel group-bys are exactly what columnar OLAP does well and OLTP does badly.

**3. Lakehouse (the source of truth + ad-hoc/ML substrate), 14.1.**
- *Choice:* the **open lakehouse** of 14.1, every event in **bronze** (raw, retained, replay source) refined to **silver** (cleaned, sessionized, identity-resolved) and **gold** (behavioral marts: per-user event tables, daily funnel/retention aggregates) as Iceberg/Delta tables. Arbitrary SQL and ML read silver/gold directly; this is the **source of truth** that back-corrects the OLAP store (the back room's complete evidence vault).
