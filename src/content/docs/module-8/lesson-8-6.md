...
| **Search** | Sometimes — relevance can be product | **Hybrid**: buy/host the engine (Elastic/OpenSearch/Algolia — Lessons 3.12, 5.6), *build the relevance layer* if search quality drives revenue. |
| **Observability** | No | **Buy early; revisit at scale** — the canonical crossover case, worked below. |
| **ML platform** | The models and data differentiate; the orchestration doesn't | **Assemble/buy** (SageMaker/Vertex/Databricks; serve LLMs via API until token spend crosses the self-host crossover — Lesson 5.15). Build the data moat, not the pipeline plumbing. |
...
- **The v1 build:** 6 engineers × ~12 months to reach parity *on the slice you actually use* ≈ **$1.5M one-time** — while still paying the vendor, because you can't go dark on observability mid-migration (Lesson 8.3's parallel-run discipline applies to vendor exits too).
...
- **Search indexes:** derived data — rebuildable from your source of truth in hours; lowest exit pain, *provided* the index-build pipeline (Lesson 3.12) is yours.
...
> Adaptation: Evaluation is the **reversibility audit** — stress the *decision* the way 5.13 stressed a design. Three probes: lock-in, triggers, and the failure modes of each verdict.
...
What I keep — the differentiation verdict, the crossover number, the exit triggers — and what I hand off, with stated priors, *is* the altitude. Lesson 8.7 applies this whole memo to one concrete platform case (Backstage vs in-house IDP); Lesson 9.12 is the behavioral pairing — telling a build-vs-buy decision you actually owned, including the one you got wrong.
...
*End of Lesson 8.6. Build vs buy is the migration problem (8.3) decided in advance — the seam and export you create at signing are what make the 8.3 playbook runnable if the exit trigger ever fires. Next: 8.7 applies this memo to a live platform case — Backstage vs an in-house developer portal — and 9.12 pairs it with the behavioral telling of a build-vs-buy call you owned.*
