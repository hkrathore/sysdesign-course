...
- **Tiering and retention thresholds:** *"Platform tunes the hot/cold age boundary and replication factors against the observed access skew and the latency SLA (service-level agreement); I own the principle, recent data hot and heavily replicated, aged data cheap, rebuildable from deep storage."* What I keep, **the real-time/historical split, immutable pre-aggregated segments, scatter-gather, bounded-cardinality rollup, and the serving-store/warehouse complementarity**, is the altitude.
