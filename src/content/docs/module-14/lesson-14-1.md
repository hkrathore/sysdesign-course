---
title: "14.1 - Quality Engineering for System Designers"
...
> Quality at Director altitude is not a QA phase: it is an **operating system**, platform plus process plus incentives, that makes safe shipping the default across forty teams. The test pyramid is a **capital-allocation map** (~70/20/10), and for distributed systems it reshapes into a trophy: the units all pass while the system breaks at the **seams**. The decision the lesson turns on is the **total cost of a test** (write, run, maintain, flake): at 1% flake a clean run is the exception and the suite gates nothing.
...
The interview prompt is *"design the test strategy for checkout."* Checkout spans a **cart** service, a **pricing/promotions** service, an **inventory** service, a **payment** service (fronting Stripe), an **order** service, and a web UI. The IC (individual contributor) instinct is "high coverage everywhere, lots of UI tests." The Director allocates by ROI, and the ROI is concentrated at one boundary.
...
- **Ignoring test maintenance and flake cost.** A 1% flake rate at scale means a clean run is the exception and the suite gates nothing; flake is an SLO (service-level objective) with an owner, not a "fix it later."
...
*End of Lesson 14.1. Quality is an operating system that makes safe shipping the default: invest by ROI per layer, expect the units to pass while the seams break, and make the paved road the easy road.*
