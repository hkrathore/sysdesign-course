...
> Split a monolith into services and the bugs move to the **seams**: the disagreement between what one team sends and what another now accepts, which no unit test can see. Consumer-driven contracts verify that agreement in each team's own CI, a 4-second job replacing a flaky nightly end-to-end run, and the real payoff is **decoupled deploys**: teams go from one weekly coupled train to shipping independently daily. The judgment call throughout is flake versus fidelity, and the deploy-order rule (provider ships first, expand-then-contract) keeps the seam safe between releases.
...
- **Backward compatibility** means a *new* provider still serves *old* consumers, you only **add** optional fields and new endpoints, never **remove** or **rename** or **tighten** an existing one (the supplier still honoring the old form). This is the property that lets the provider deploy first.
