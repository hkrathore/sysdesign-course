---
title: "4.15 - Distributed Job Scheduler"
...
*End of Lesson 4.15. This assembles the building block from 3.15 into a full RESHADED design, and the lesson is that **the R and E steps reframe the whole problem**: a scheduler is not a read-serving system but a control plane whose load is the due-poll and the herd, which is why we size the timer table and the midnight spike, not a cache. Next: 5.15 ChatGPT / LLM serving, the GPU-bound, batched-inference problem.*
