...
**Clarifying questions a Director asks first** (each changes the design): How skewed is the follower distribution, is there a power-law tail of celebrities? (Yes, it forces hybrid.) Reverse-chron or ranked? (Reverse-chron v1.) How stale can a timeline be? (Seconds.) DAU (daily active users) and average followers? (Below.) Do we own media storage or offload to a blob store? (Offload, the blob-store building block.)
...
**Write QPS (queries per second) (tweets):**
...
| **Media** (images/video) | Write-once, read-heavy, large objects, served via CDN | **Blob/object store + CDN** | **S3** + CloudFront | Bytes don't belong in the tweet store; store a pointer, serve from edge. *Rejected:* BLOBs (binary large objects) in the database, bloats it and kills cache density. |
...
The decision falls straight out of the access patterns established in R/E, write-heavy immutable tweets → LSM (log-structured merge) wide-column; regenerable hot lists → cache; two-directional adjacency → indexed KV (key-value). **tweet_id is generated by a Snowflake-style sequencer**: a 64-bit, **time-sortable** ID, which we will exploit so that "merge timelines by recency" is just "sort by ID."
...
*Stress the design against the NFRs (non-functional requirements); find the bottlenecks; fix each, naming the trade-off the fix buys.*
...
925K inserts/s across a Redis cluster is real cross-AZ (availability zone) traffic and CPU, the **fan-out amplification** cost from **The publish-subscribe building block**. We don't re-derive it; we **govern** it: skip-inactive and the celebrity threshold are themselves the two biggest amplification controls, and we watch **fan-out lag** and **per-node insert rate** as SLOs (service-level objectives).
