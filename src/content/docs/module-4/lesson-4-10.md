...
- **Pre-position the whole catalog at the ISP edge (Open Connect).** Demand is predictable, so Netflix ships appliances into ISPs and **pre-loads tonight's likely-watched titles overnight** (pallets stacked at the newsstand before the rush), peak-evening traffic is served from a box inside your ISP. YouTube's long-tail catalog can't be fully pre-positioned, so it leans on a general CDN + caching.
