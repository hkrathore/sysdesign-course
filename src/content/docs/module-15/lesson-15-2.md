---
title: "15.2 - Observability in Anger"
...
> Monitoring answers the questions you thought to pre-wire; **observability lets you ask a new question of a live system mid-incident**, and that difference is a 20-minute localize versus a 4-hour archaeology dig. The stack is a division of labor: metrics say *what*, logs say *why*, traces say *where*, profiles say *which line*, all threaded by one **trace ID**. The decision it turns on: instrumentation is bought **before** the outage, and one unbounded metric label (`user_id`) multiplies series into the billions and detonates the bill.
...
*End of Lesson 15.2. Observability is the ability to ask new questions of a live system mid-incident; the Director invests in RED/USE localization and trace-ID correlation before the outage, and budgets for cardinality and cost so the answer is there when it counts.*
