...
- Reason about **severity** as a pre-agreed trigger table (SEV1–SEV4 with explicit thresholds) so declaring is a *lookup*, not a judgment call made by an anxious engineer, and so the bias against declaring (ego, optimism) stops costing you minutes of MTTR (mean time to recovery).
...
- **Incident Commander (IC), the coordinator and decision-maker, explicitly not hands-on-keyboard.** The IC (individual contributor) holds the picture, drives the cadence, decides between rollback and forward-fix, decides when to escalate, decides when the incident is over. The single most common and most damaging failure is the IC dropping into the logs to debug, because the moment they do, nobody is coordinating and the room reverts to a crowd. For a SEV1 the IC is often a senior engineer or EM (engineering manager) trained in the role, *not* necessarily the most senior person present and *not* the subject expert (the expert is too valuable on the keyboard to also coordinate).
...
- **SEV2**, major: significant degradation, a key feature down, elevated error rates breaching the SLO (service-level objective) burn-rate threshold, but the core path limps. Page the on-call and the IC; Comms on standby.
...
*End of Lesson 15.4. Run the war room like a fire ground: one commander who decides and doesn't hold a hose, declared early, on a steady drumbeat, built so the org fights the fire without you there.*
