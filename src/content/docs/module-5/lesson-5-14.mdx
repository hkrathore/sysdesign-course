...
- *Latency bar?* → **p99 < 1 ms intra-AZ** (availability zone) for the cache; single-digit ms cross-AZ. A cache that adds 10 ms isn't a cache.
...
4. **Evict** under memory pressure (cache mode): TTL (time-to-live) + LRU.
...
**Bandwidth:** `1M GET/s × 10 KB ≈ 10 GB/s` aggregate read egress, ~500 MB/s/node, well within a 25 GbE NIC (network interface card). Fine *unless* it concentrates on one hot key (the failure mode, not the average).
...
- *Rejected, a B-tree / LSM (log-structured merge) on disk (RocksDB-backed):* buys durability and range scans we don't need, and pays ~100 µs-1 ms disk latency that defeats the p99 < 1 ms bar. Correct for a durable KV at scale where the working set exceeds RAM; **wrong for a latency-first cache** where the whole point is staying in memory.
...
**The value record** (per key, per node): `key → { value: bytes, version, ttl_expires_at }`. The `version` is a **vector clock** (or a simpler last-writer-wins timestamp for cache mode), it's the metadata that lets divergent replicas reconcile. For pure cache mode where staleness is harmless, **LWW (last-write-wins) by timestamp** is enough and cheaper; for KV mode where two clients may write concurrently, **vector clocks** preserve the conflict for the application to resolve.
...
- **Multi-region goes active-active eventual.** Each region owns a full ring; writes replicate cross-region async (CRDT-friendly (CRDT = conflict-free replicated data type) LWW for cache data). *Trade-off:* cross-region staleness of tens to hundreds of ms, acceptable for cache, **not** for a global session store, which would need a region-pinned strong-consistency tier or a consensus layer (consensus-layer territory). Name that the consistency posture **can't** be uniform across regions and the cost of making it so.
...
- **The eviction internals:** *"The cache team owns whether we run Memcached slab/LRU or Redis with LFU (least frequently used) eviction; my prior is Memcached for a pure look-aside cache because its memory efficiency and operational simplicity win, Redis only if we need its data structures. I want their benchmark on our value-size distribution before we commit."*
