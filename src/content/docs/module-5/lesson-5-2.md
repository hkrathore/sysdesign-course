...
Saga in state DEBIT_COMMITTED / CREDIT_PENDING. Sweeper finds the record after TTL (~30 s), retries the credit (idempotent). If the receiver shard is permanently down, the sweeper compensates: re-credits the sender; transfer marked FAILED (the courier returns the cash to the first vault). *Trade-off:* the sender sees money returned after a delay, acceptable vs leaving both shards locked under 2PC.
