...
This split is the whole game. The desk handles millions of *lookups* with a tiny data footprint; the floors handle petabytes of *bytes* with almost no logic. They scale on **different axes** and you size them independently — the desk for request rate, the floors for raw capacity. And one more rule that makes object stores cheap and simple: **you don't edit a parked car in place.** You can't repaint the left fender of a car sitting in space F-23. To change an object you drive in a whole new car and the ticket now points at the new space; the old one is reclaimed later. Objects are **immutable, whole-object writes** — and that single constraint, which a database would never accept, is exactly what lets the data plane be dumb, append-only, and trivially replicated.

Hold that image. Everything below — metadata vs data plane, why writes are whole-object, how durability is bought with parity instead of copies, how a 5 TB upload is chopped into parts, why cold data costs 23× less — is a literal feature of that garage.
...
**Why split at all — the rejected alternative.** The naive design is "just put the bytes *in* the database" — store the object as a BLOB column in Postgres next to its metadata. **Rejected**, and decisively: a relational DB is built for small rows, in-place updates, transactions, and rich queries — paying B-tree, WAL, MVCC, and buffer-pool costs (Lesson 2.3) you do not need for a 4 GB immutable video. Multi-GB rows wreck the buffer pool, replication, and backup; you'd be paying transactional-database dollars per GB to store data that is written once and never updated. Conversely, putting *metadata* on the dumb storage nodes means every `LIST` or permission check scans the data plane. The split lets each side use the right tool: a **strongly-consistent small-record store** for the namespace, **dumb append-only capacity** for the bytes. This is exactly how the real systems are built — **Azure Blob Storage** separates a **partition layer** (metadata/index) from a **stream layer** (the append-only byte store) in the WAS design; **Google Colossus** (the GFS successor) separates metadata **curators** from **D** chunk servers. **Ceph/RADOS** is the instructive outlier: it keeps cluster *membership* in a small consistent map but then **computes** each object's placement algorithmically via **CRUSH** (a hash of the key over the cluster map) rather than storing a per-object location in a metadata service — trading a central index lookup for deterministic, lookup-free placement. Both shapes work; the constant is a **small consistent control structure** governing where the **dumb byte capacity** puts things.
...
This buys cheap `LIST prefix` (a range scan within a partition) but inherits range-partitioning's classic hazard: **sequential keys create a hot partition.** Keying objects by a monotonically increasing timestamp (`2024-07-29T12:00:00.001`, `...002`, …) drives every write into the *last* partition — one node melts while the rest idle (the same hot-shard failure as a monotonic primary key in 2.5). The historical fix was to **inject a random/hashed prefix** to spread writes; modern S3 **auto-partitions** under load and now sustains **3,500 PUT/COPY/POST/DELETE and 5,500 GET/HEAD requests per second per prefix**, with **no limit on the number of prefixes** — so you scale read/write throughput by *spreading keys across many prefixes* (10 prefixes ≈ 55,000 GET/s). The Director-level point: **request rate per prefix is the scaling unit**, and a bad key schema (everything under one hot prefix) is a self-inflicted bottleneck no amount of storage hardware fixes.
...
- **The repair-cost fix — local reconstruction.** **Microsoft Azure Storage** uses **Local Reconstruction Codes (LRC)** — e.g. **(12, 2, 2)**: 12 data fragments, 2 *local* parities, 2 *global* parities. By adding local parity groups, a single-fragment repair reads only its **local group (≈6 fragments)** instead of all 12 — cutting repair I/O — while keeping overhead at **≈1.33×**. LRC is a deliberate trade: slightly different math from pure Reed-Solomon to make the *common case* (single-disk failure) cheap to repair.
...
Modern object stores give **strong read-after-write consistency**: after a successful `PUT` returns, every subsequent `GET` (and `LIST`) sees the new object or new version — globally, in-region, no stale window. **S3 has been strongly read-after-write consistent for *all* operations — new PUTs, overwrite PUTs, DELETEs, and LIST — since December 2020**, at no extra cost. Before that, S3 gave read-after-write for *new* objects but only **eventual** consistency for **overwrites and deletes** (and listings), which forced ugly workarounds (EMRFS consistent view, S3Guard) in data-lake pipelines. Naming this correctly is a sharp signal — many candidates still describe S3 as "eventually consistent," which has been wrong for years.

**Why is strong consistency affordable here when it's so costly for a general database?** Because **objects are immutable, whole-object writes.** There are no partial updates, no read-modify-write, no multi-key transactions, no conflicting concurrent edits to *part* of an object. A write is atomic at the object level: the data plane stores the new blob's fragments, and the **commit is a single metadata flip** — the index entry for `bucket/key` atomically points at the new version. A `GET` reads the metadata (strongly-consistent KV lookup), gets the current location, fetches bytes. Because the *only* thing that needs to be consistent is the **small metadata pointer**, you get strong consistency from the metadata store's own consistency (a quorum write / consensus on a tiny record, Lesson 2.8) without paying it across petabytes of bytes. Contrast a relational DB, where strong consistency must hold across in-place row mutations, indexes, and transactions. **Immutability is the simplification that buys cheap strong consistency** — that's the causal chain to state out loud.
...
2. **Upload parts in parallel** — each part is **5 MiB minimum** (the last part may be smaller) and **5 GiB maximum**, up to **10,000 parts** per object, giving the **5 TiB** max object size. Each part is its own request with its own checksum and returns an `ETag`. Parts upload concurrently across many connections, and a **failed part is retried independently** — you re-send one 64 MB part, not the whole object.
...
Meanwhile a **background fleet** continuously runs **scrubbing** (read fragments, verify checksums, detect bit rot / failed disks) and **repair** (rebuild lost replicas/fragments to restore the durability level). This background work is the *real* cost of eleven nines, and a Director should name it: durability isn't a static property, it's a **continuous repair process you capacity-plan and monitor** — exactly the operational framing from the LSM-compaction discussion in 2.3.
...
- **Durability tiering — the core cost call.** **Hot (recent) media:** **3× replication** — reads are frequent and latency-sensitive, repair must be instant, and the data is comparatively small in aggregate, so the 200% overhead is worth it. **Cold (aged-out) media:** transition to **erasure coding RS(10,4)** — **40% overhead instead of 200%**. On 500 PB of cold data that's the difference between buying **~700 PB** (1.4×) and **~1,500 PB** (3×) of disk — *roughly 800 PB of hardware avoided*. We accept EC's higher reconstruction cost on the cold tier because those objects are rarely read; this is precisely the f4 motivation. *Rejected:* 3× everything (simple, but indefensible cost at exabyte scale); EC everything (kills storage cost but loads the hot path with reconstruction CPU/IO and hurts small-object and tail-read latency).
...
- **Quoting durability without the overhead math** — "three copies" is not an answer; 200% (replication) vs 40% (EC) and the failures-tolerated count is.
- **Calling S3 eventually consistent** — it's been **strongly** read-after-write for all operations since December 2020.
- **Erasure-coding everything** — EC's reconstruction CPU/IO and small-object penalty make replication better for hot/small data; tier the durability scheme.
...
- **Tiering reflexively to "save money"** — ignoring retrieval latency, per-GB retrieval fees, and 90/180-day minimums that can exceed the savings.
- **Forgetting incomplete multipart uploads** — orphaned parts you keep paying for; abort them via lifecycle.
- **Expecting database semantics** — no cross-object transactions, no in-place edit, no atomic rename (it's copy-then-delete).
...
> *Model:* Multi-GB immutable videos are the **wrong shape** for a relational engine. Postgres is built for small rows, in-place updates, MVCC, and transactions — it pays B-tree, WAL, and buffer-pool costs (2.3) that buy nothing for write-once blobs, and large BLOBs **destroy the buffer pool, replication, and backups** while costing **transactional-database dollars per GB**. The right design **splits the planes**: keep the small, hot, strongly-consistent **metadata** (key, size, owner, ACL, location) in a sharded KV/Postgres catalog, and put the bytes on a **dumb, append-only data plane** scaled by capacity. "Transactional together" is a non-requirement — objects are immutable whole-object writes, so a single atomic **metadata commit** already gives strong read-after-write consistency, without paying transactions across petabytes. We lose nothing real and gain independent scaling and 10×+ lower $/GB.

**Q3.** Explain S3's consistency model and why it can offer it cheaply when a general database can't.
> *Model:* S3 is **strongly read-after-write consistent for all operations — new PUTs, overwrites, DELETEs, and LIST — since December 2020** (before that, overwrites/deletes/listings were only eventually consistent). It's cheap *because objects are immutable, whole-object writes*: there's no partial update, read-modify-write, or multi-key transaction to make consistent. A write durably stores the new blob, then **flips a single metadata pointer** atomically; the only thing requiring consistency is that **tiny pointer**, which the metadata store handles with a quorum/consensus write on a small record (2.8). A relational DB must hold strong consistency across **in-place mutations, indexes, and transactions** — far more expensive. **Immutability is the simplification that buys cheap strong consistency.** The boundary: S3 deliberately offers no cross-object transactions, conditional content updates, or atomic rename — those stay a database's job.
