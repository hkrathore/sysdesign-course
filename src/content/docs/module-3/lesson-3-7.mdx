...
> A distributed cache is a second copy of the truth held in RAM, and it lives or dies on one number: the hit rate. At 50,000 read QPS a 90% hit rate means the database sees 5,000, a 10x cut, at ~1 ms instead of ~10 ms; misses, not hits, size your origin. The decisions are the levers that move that number (eviction, sharding, stampede control) and the boundary: which data you refuse to cache because a stale read is a bug, not a latency win.
...
- Engineer **replication and failover** for the cache, and decide whether the cache even *deserves* HA (high availability).
- Defend against **hot keys** and **cache stampede / thundering herd** with request coalescing, TTL (time-to-live) jitter, and early refresh - and state the **consistency cost** of caching authoritative data.
...
Take a read-heavy service at **50,000 read QPS** (queries per second). An origin Postgres read costs ~**10 ms**; a Redis hit costs ~**0.5-1 ms** (the latency hierarchy). Put a read-through cache in front at a **90% hit rate**:
...
A cache is finite memory; the **working set** almost never fits the whole keyspace. When memory is full, the eviction policy decides what to discard (which plate leaves the lamp) - chosen from the **access pattern**, because it directly moves the hit rate:
...
One cache node has a memory ceiling (say **64 GB** usable) and a throughput ceiling; a 500 GB working set or 1M+ QPS needs a **fleet**, and the fleet needs a key→node rule **every client agrees on** (every plate needs its home counter), or you cache the same key in five places and serve stale copies.
...
- **Request coalescing (single-flight):** the first miss takes a short lock, fetches, and populates; concurrent requests **wait for that one fetch**. 10,000 misses collapse to **1** origin read (one order to the kitchen). (Trade: waiters block briefly; give the lock a TTL so a crashed fetcher can't wedge it.)
