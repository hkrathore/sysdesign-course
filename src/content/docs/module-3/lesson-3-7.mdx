...
A distributed cache is an in-memory key-value tier - **Redis** or **Memcached** (the key-value family from Lesson 2.2) - sitting between the stateless app tier (2.10) and the origin database. It earns its place on one number: the **hit rate**.

#### Why a cache, in numbers (the leverage)

Take a read-heavy service at **50,000 read QPS**. An origin Postgres read costs ~**10 ms**; a Redis hit costs ~**0.5-1 ms** (the latency hierarchy of 1.4). Put a read-through cache in front at a **90% hit rate**:
...
The naive rule, `hash(key) mod N`, has a fatal property for a cache: change `N` and **almost every key remaps** (~90%+ going 10→11 nodes - Lesson 2.6's numbers). Every remapped key is a **miss**, so scaling the fleet flushes the cache and stampedes the origin at the worst moment - you're scaling *because* you're under load. **Consistent hashing** (2.6) bounds the damage: adding or removing a node moves only ~`1/N` of the keyspace, so a capacity change costs a small, bounded miss blip instead of a full cold-load surge. Virtual nodes smooth the distribution and weight bigger machines.
...
A cache buys latency by holding a copy, and the copy can be **wrong** - the PACELC Else-side (2.7) made concrete: lockstep with the database costs latency; drift serves **stale** data.
...
Continue the photo/feed app from Lessons 2.2 and 2.10. The read path is ~**100:1** reads:writes on timelines, so caching is the load-bearing decision.
