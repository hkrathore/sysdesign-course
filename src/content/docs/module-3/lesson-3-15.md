...
### Learning objectives
- Decompose a scheduler into two separable concerns — **deciding *when* a job is due** (the scheduler/timer) and **running it reliably** (the executor) — routed through a durable queue so you **reuse the messaging-queue machinery** (Lesson 3.8) instead of re-deriving it.
...
- **A durable queue (the seam):** the scheduler enqueues a "run this now" message (SQS/Kafka — Lesson 3.8).
- **The executor (worker plane):** a stateless worker pool consumes the queue with **all of 3.8's machinery** — visibility timeout, acks, retries, and a **dead-letter queue** for poison jobs.
...
| **Redis ZSET** (`ZRANGEBYSCORE timers -inf <now>`) | O(log N + M) | no — AP, async-replica loss window (3.7) | very high, in-memory | hot index over a durable store; sub-second due-set |
...
The CAS lives in the *store*, not the queue — a vanilla queue like SQS won't fence by token on its own, so the claim row (or an outbox table) is where the epoch check happens. Same conditional-write machinery as 2.8/3.4.
...
- **Stuck-run detection:** a job stuck in `running` past its expected duration (worker died mid-job) needs a timeout → DLQ → page, exactly as 3.8's visibility timeout handles a dead consumer.
...
- **"How do you scale past one scheduler node / one store?"** — *Strong:* **partition the timer space** by `job_id` (consistent hashing, 2.6) so each scheduler owns a shard, naming **rebalancing on churn** as the cost; knows the decentralized-contention option and that the **shared lock** is its ceiling. *Red flag:* "add more scheduler instances" with no story for double-firing or the lock bottleneck.
...
- **"Where do you keep the timers?"** — *Strong:* **durable replicated store as source of truth**; an in-memory index (ZSET / timing wheel) only as an acceleration layer rebuilt on restart. *Red flag:* in-memory timers, or Redis as the sole record (3.7 loss window).
...
- **In-memory or Redis-only timers** — a restart or async-replica loss drops future jobs; the durable store is the source of truth (3.7).
...
### Key takeaways
- **Split the scheduler from the executor** via a durable queue: the scheduler only moves a due job `scheduled → enqueued`; the queue + stateless workers own execution (retries, backoff, DLQ — Lesson 3.8). This isolates blast radius and reuses, not rebuilds, the delivery machinery — Airflow's scheduler/executor/worker split is this shape.
- **Timers live in a durable, replicated store as the source of truth** — never RAM-only or Redis-only (3.7). An in-memory ZSET or timing wheel is an acceleration **index** rebuilt on restart; the hot operation is polling due timers with a row lock (SKIP LOCKED pattern).
- **Leader election (delegated to etcd/ZooKeeper) prevents *concurrent* double-fire — and nothing more.** A failover gap (≈ lease TTL) can miss fires; **epoch fencing makes a stale leader's writes rejectable** (2.4). Firing is therefore a choice: **at-least-once (catch up)** vs **at-most-once (skip)**.
...
- **Scale by partitioning the timer space** (consistent hashing by `job_id`, 2.6 — cost: rebalancing on churn); **quantify the thundering herd** at round cron times and **smear with jitter**; and **monitor for silent misses** (dead-man's switch), because a job that *didn't* fire raises no alarm by itself.
