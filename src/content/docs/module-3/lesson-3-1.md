...
Your client makes **one recursive query** ("hand me the final answer") to its recursive resolver (ISP, `8.8.8.8`, `1.1.1.1`), and the **resolver iterates the tree, root → TLD → authoritative, on the client's behalf** (the operator working the chain of directories), then caches the result. A cache-cold lookup costs the resolver ~3 upstream round trips; every lookup within the TTL costs **zero**, and answers are cached at every layer (browser, OS stub, recursive resolver), with most hits landing at the shared recursive resolver. That caching is why DNS survives at internet scale; the root and TLD servers would melt otherwise.
...
The authoritative server can return **different answers to different resolvers** (different numbers to different callers). That is the entire basis of DNS-based global traffic management. The four mechanisms (mapped to **Route 53** policy names, which is the lingua franca in interviews):
...
- **"Propagation" is a misnomer, there is no push.** Changing a record doesn't *send* anything anywhere; it just means new lookups get the new answer once **old cached entries expire** (the sticky notes reaching their expiry). Worst-case propagation ≈ **the TTL that was in effect when the entry was cached** + resolver slop. Lower the TTL *before* the change, not during.
