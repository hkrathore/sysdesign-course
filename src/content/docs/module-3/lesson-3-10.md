...
> A rate limit is really **two numbers, not one**: a sustained rate and an allowed burst, and the algorithm you pick is mostly which of those knobs it gives you. The hard part is distributed enforcement: a fleet of gateways sharing one global limit through an atomic Redis Lua script, at a **~0.5-1 ms tax** on every request. The decision it all turns on: what happens when Redis dies, **fail-open** for general traffic, **fail-closed** for auth and payments; the limiter must never be more fragile than what it protects.
...
**Distributed enforcement - the actual hard part.** Everything above is a single counter. Real systems run **many limiter instances** (every gateway pod, every edge node) that must enforce **one global limit** (a thousand doors, one shared clicker), and there are two routes. **Local per-instance counters** need no coordination, but **N instances each enforcing "1,000/min" is a real global limit of N × 1,000** - acceptable only as a coarse first layer, or by dividing the limit by instance count (brittle under autoscaling). A precise global limit needs a **shared store - Redis is the canonical choice**: in-memory (~**0.5-1 ms** same-DC), single-threaded per shard, with the data structures you need. The cost is that **every rate-limited request now does a network round-trip to Redis** (every bouncer phoning the central office) - that 0.5-1 ms rides your request path, and Redis becomes a dependency whose availability gates your throttled endpoints.
...
- **"You need a single global limit (a third-party quota). Where does this break?"** - *Strong:* the **hot-key ceiling** - one counter = one key = one shard, and you can't shard a single hot key; mitigations are sharded counters or local approximation. *Red flag:* thinks "just add Redis shards" helps a single key.
...
- **Dimension** by key (authenticated fairness/billing), IP (anonymous only - NAT and rotation make it coarse), or cost-weighted (expensive endpoints). A **single global limit** hits the **hot-key ceiling** - one key, one shard, unshardable - fixed by sharded counters or local approximation.
