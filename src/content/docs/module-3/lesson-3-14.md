...
The critical move is **what you alert on**. The wrong way, and the single biggest source of alert fatigue, is alerting on **causes**: "CPU > 80%", "a pod restarted", "disk 70% full". Most never affect a user, and they page someone at 3am for nothing. The right way is to alert on **symptoms the user feels, via the rate you're burning the error budget**. The policy, stated once: **page on fast burn (~14.4× the even-pace rate over 1 hour, you're about to spend a meaningful chunk of the month's budget before breakfast), ticket on slow burn (~1× over 3 days, fix it in business hours)**, with each alert gated by both a long and a short window so it fires fast when an incident starts and resets fast when it ends.
