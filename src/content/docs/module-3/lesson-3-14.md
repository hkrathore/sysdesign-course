...
> Monitoring is three instruments on one cost axis: metrics detect (cheap, ~1-2 bytes a sample), traces localize, logs root-cause (expensive), and an incident walks them in that order. The decision the lesson turns on is what you page on: symptoms via **SLO burn rate** (fast burn at 14.4× pages, slow burn tickets), never causes like CPU spikes. The number that kills these systems is **cardinality**: one unbounded label like `user_id` multiplies series by millions, OOMs the TSDB, and detonates a per-series SaaS invoice overnight.
...
The strong-signal sequence in an incident is **metric → trace → log**: a metric alert tells you checkout error rate jumped; a trace tells you the failing requests all stall in the `payment-gateway` span; the logs on that service tell you it's a TLS handshake timeout to Stripe. Using the wrong pillar is expensive, alerting off logs means parsing gigabytes to compute a number you could have had for ~2 bytes as a counter; trying to root-cause from a metric is impossible because a number carries no context (the vital sign can't explain itself).

**Metrics and the time-series database (TSDB).** A metric is a named numeric series with a set of **labels** (dimensions): `http_requests_total{service="checkout", method="POST", status="500"}`. Each unique combination of label values is **one time series**, stored as a stream of timestamped samples. TSDBs (Prometheus, and its horizontally-scaled backends Thanos / Cortex / Grafana Mimir) exploit the regularity of time-series data to **compress a raw 16-byte sample to ~1-2 bytes**, the order-of-magnitude win that's *why* you use a TSDB and not Postgres for this. Metric types in one line: **counters for totals** (you rate them), **gauges for in-flight values** (queue depth, memory), **histograms for percentiles**, the only honest way to get a fleet-wide p99, because **you can never average percentiles** across hosts. A Director who quotes average latency while p99 is 4 seconds has missed the tail the user actually feels.

**Pull vs push, the collection-model decision.** How do metrics get from a service into the TSDB? **Pull** (the Prometheus model): the TSDB **scrapes** each target's HTTP `/metrics` endpoint on a fixed interval (commonly every **15 s**), with the target list coming from service discovery. **Push** (StatsD, the OTel Collector, Datadog agent): each service **sends** its metrics to a collector. The decision turns on three things, and you state the trade, not a preference:

1. **Liveness for free (pull's killer feature).** With pull, a **failed scrape is itself a signal**, Prometheus records `up == 0`, so you detect a *dead* instance with zero extra instrumentation. Push **cannot distinguish "dead" from "merely silent."** This is the single strongest reason pull dominates the cloud-native world.
2. **Service discovery and control live server-side** with pull (the monitoring system decides who to scrape and the cadence), versus push putting that burden on every client.
3. **Short-lived and network-isolated jobs break pull**, a batch job that runs for 4 seconds is gone before the 15 s scrape; a target behind a NAT (network address translation) can't be reached. The Prometheus answer is the **push-gateway**, a deliberate **exception, not the default**: route normal service traffic through it and you get a stale-metric trap and a centralized bottleneck. **Rejected default:** making everything push "because it's simpler for the app", you forfeit free liveness detection.

One clarification that earns signal: **OpenTelemetry (OTel) is instrumentation and collection, not a backend.** It's the vendor-neutral SDK + Collector + wire protocol that emits metrics/traces/logs to *whatever* backend you choose, the **anti-lock-in** answer (instrument once, switch from Datadog to self-hosted without re-instrumenting), but it does not store or query anything itself.
...
- Bounded and fine: `service` (50 values) × `method` (~5) × `status` (~10) × `instance` (200 pods) = **500,000 series**. Large but tractable.
- Catastrophic: add `user_id` as a label with **10 million** users, and you've multiplied by 10⁷, a series *per user per combination*, tens of billions of series. The TSDB falls over.

The classic self-inflicted blowups: a **raw URL path** (`/user/12345/orders/98765`) in a label instead of the **templated route** (`/user/:id/orders/:id`); labeling by `request_id`, `email`, `session_id`. **Labels are for bounded dimensions you'll group or filter by; per-event identifiers belong in logs and traces, never in metrics.**

Why it's the *cost* driver and not just a performance footnote: each active series consumes memory in the TSDB ingest path (on the order of low-KB of RAM per series), and a single Prometheus node strains in the **low tens of millions of active series**, past that you shard with Mimir/Cortex or cut cardinality. And, the Director punchline, **the SaaS pricing model is cardinality.** Datadog and New Relic bill **per custom metric, where a custom metric is a unique series**. A cardinality blowup therefore doesn't just OOM (out-of-memory) your Prometheus; it **detonates your invoice**, a single careless `user_id` label has turned a $40k/year observability bill into a six-figure one overnight. Controlling cardinality (drop high-cardinality labels at the collector, enforce metric-naming review) is a budget lever, which is exactly why it's a Director's problem and not the on-call engineer's.

**What to measure, pick one framework.** You don't invent metrics ad hoc; you apply a method, and naming the method is itself signal. **Pick one framework and apply it consistently; the Four Golden Signals (Google SRE), Latency, Traffic, Errors, Saturation, is the most common** for any user-facing service, with resource-level views underneath for the machines and pools. A Director who frames a dashboard that way sounds like they've run on-call; one who lists "CPU, memory, disk" sounds like they've watched a server, not a system.

<details>
<summary>Go deeper, Golden Signals vs RED vs USE (IC depth, optional)</summary>

- **Four Golden Signals (Google SRE):** Latency, Traffic, Errors, Saturation, the general-purpose default for any user-facing service.
- **RED:** Rate, Errors, Duration, the same idea sharpened for request-driven services (APIs, microservices); essentially Golden Signals minus saturation.
- **USE:** Utilization, Saturation, Errors, for a *resource* (CPU, disk, a connection pool), not a service.

The composition that works in practice: RED/Golden Signals for the service surface, USE for the fleet and dependencies underneath. They are restatements of one idea, measure what the user experiences at the service layer, measure what's running out at the resource layer, so the choice between them matters far less than applying one consistently across every service.
...
- The **error budget** is the inverse: `1 − SLO`. For 99.9% over 30 days that's **43.2 minutes** of "allowed" badness. The budget reframes reliability from "never fail" (impossible, and you'd never ship) to "fail at most this much", a quantity you can *spend* on releases and risk.

The critical move is **what you alert on**. The wrong way, and the single biggest source of alert fatigue, is alerting on **causes**: "CPU > 80%", "a pod restarted", "disk 70% full". Most never affect a user, and they page someone at 3am for nothing. The right way is to alert on **symptoms the user feels, via the rate you're burning the error budget** (page the nurse when O₂ drops). The policy, stated once: **page on fast burn (~14.4× the even-pace rate over 1 hour, you're about to spend a meaningful chunk of the month's budget before breakfast), ticket on slow burn (~1× over 3 days, fix it in business hours)**, with each alert gated by both a long and a short window so it fires fast when an incident starts and resets fast when it ends.

<details>
<summary>Go deeper, the burn-rate arithmetic (IC depth, optional)</summary>

Burn rate is how fast you consume the budget relative to the even pace that would exactly exhaust it over the window. Burn rate 1 = spend exactly the whole budget over 30 days; burn rate 14.4 = exhaust the 30-day budget in ~50 hours. The canonical multi-window, multi-burn-rate policy (Google SRE) for a 99.9% / 30-day SLO:
...
The arithmetic ties out: fraction burned = (window ÷ 720h) × burn rate, so 1/720 × 14.4 = 2%, 6/720 × 6 = 5%, 72/720 × 1 = 10%. The multi-window refinement: each alert requires **both** the long window and a short window (e.g. the 1h/14.4× page also requires the last 5 minutes to still be burning) so it doesn't flap or keep paging after recovery.

</details>

The Director framing of signal-vs-noise is an **on-call-health and risk decision**, not a tooling preference: the metric you manage is **pages per shift and the % that are actionable**. Every non-actionable page erodes responder trust and raises the odds the *real* 3am page is dismissed as noise, a reliability risk created *by* your monitoring. **Rejected alternative:** the "alert on everything, better safe than sorry" posture, it reliably produces a team that has muted the pager, strictly worse than fewer, symptom-based, budget-driven alerts. Cause-level signals go to **dashboards** (looked at during an investigation); **pages** are reserved for "the user is being hurt fast enough that the budget is in danger."

**Dashboards.** Dashboards are for **humans investigating**, not for machines deciding, that's the alert's job. A good hierarchy: a top-level **SLO / Golden-Signals** view per service (is the user okay, how much budget is left), drilling into resource views for the fleet and dependencies. The anti-pattern is the "wall of 200 graphs" nobody reads, every panel should answer a question someone actually asks during an incident, or it's noise that hides the signal.
...
- **The alerts:** a **fast burn (14.4× over 1h**, here, a sustained ~1.4% error rate**) pages** on-call, gated by a 5-minute short window so it fires within minutes of onset and clears when checkout recovers. A **slow burn (1× over 3 days**, a steady ~1 bad req/s**) opens a ticket**, not a page, fix it Monday.
...
| **Trace sampling** | **Head sampling**: decide at request start (cheap, stateless), but may miss the rare error trace | **Tail sampling**: decide after the request finishes, keep all errors/slow traces; but the collector must **buffer every span** (stateful, expensive) | Head when traffic is uniform and cost-sensitive; **tail** when you must guarantee every error/slow request is captured and can fund a stateful collector |
...
- **"How do you alert so on-call doesn't drown?"**, *Strong:* alert on **SLO burn-rate / symptoms the user feels**, fast burn pages, slow burn tickets; cause-level signals (CPU, restarts) go to **dashboards**; the managed metric is **pages/shift and % actionable**, because alert fatigue is itself a reliability risk. *Red flag:* "alert on CPU > 80% and every error", the classic pager-muting machine; no SLO or budget framing.
- **"Why does your monitoring bill explode / Prometheus fall over?"**, *Strong:* **cardinality**, an unbounded label (`user_id`, raw URL, `request_id`) multiplies series; it OOMs the TSDB *and* detonates per-custom-metric SaaS pricing; fix by dropping high-cardinality labels at the collector and templating routes. *Red flag:* doesn't know what cardinality is, or thinks "just add storage."
- **"Pull or push, and why?"**, *Strong:* pull by default for **free liveness detection** (`up == 0`) and server-side discovery; push (push-gateway) only for short-lived/firewalled jobs, named as an exception with its stale-metric caveat. *Red flag:* "push is simpler" with no mention of losing the dead-vs-silent distinction.
...
- **Alert on SLO burn-rate and symptoms, not causes**, fast burn (~14.4×/1h) pages, slow burn (~1×/3d) tickets; the metric you manage is **pages/shift and % actionable**, because alert fatigue is a reliability risk. Cause-level signals belong on **dashboards**.
