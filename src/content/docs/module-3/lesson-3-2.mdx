...
> The load balancer is the most concentrated single point of failure you own: one box that can take down the N redundant servers behind it. Two decisions carry the lesson. L4 vs L7 is what the device can see: the connection 4-tuple in microseconds, or the request's path, host, and cookie at hundreds of microseconds per call. And HA by altitude: floating VIP in one data center, ECMP/anycast active-active for scale, DNS/GSLB across regions, each layer bounding the blast radius of the one below.
...
- Treat the load balancer as a **single point of failure** and make it HA (high availability) (active-passive with a floating VIP, active-active via ECMP/anycast, DNS/GSLB), and place LBs correctly across the request path (edge, internal, service mesh).
...
- **Active-active via ECMP (equal-cost multi-path) / anycast:** advertise the **same IP from multiple LBs** and let the network spread flows across all of them, every box serves, capacity scales horizontally, and a dead LB simply stops being routed to. This is how Google's Maglev and edge networks like Cloudflare run. Cost: it needs **BGP/network (BGP = Border Gateway Protocol) control** you may not have outside a cloud or your own backbone.
- **DNS / GSLB (global server load balancing):** hand out multiple records via a **health-checked GSLB** (e.g., Route 53) so clients resolve to region-local LBs. The only layer that balances **across regions** and survives a whole-site loss. Cost: **DNS TTL (time-to-live) caching** makes failover **slow and partial** (clients keep hitting a cached dead record for the TTL, often 30-60s+), so DNS is a *coarse* layer used **above** a fast in-region LB, never your only failover.
...
- **Sessions:** **stateless**, session state in **Redis**, JWT (JSON Web Token) for the client, so any backend serves any request and failover loses nothing. Sticky sessions rejected: they'd worsen the upload imbalance, and losing a backend would drop every session pinned to it.
