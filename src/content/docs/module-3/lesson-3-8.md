...
> A point-to-point queue does exactly two jobs: it decouples the producer from the consumer's failures, and it load-levels a spike against a fixed fleet (300k flash-sale orders drain through 2,000/s workers in ~150 seconds instead of forcing a 5x idle fleet). The decision the lesson turns on is delivery semantics: exactly-once delivery is impossible over a network, so you run at-least-once and make the consumer idempotent on a business key, effectively-once. Everything else (visibility timeouts, DLQs, partitions) is machinery serving those two facts.
...
- If the consumer **crashes, times out, or never acks**, the visibility timeout expires and the message **becomes visible again** (the ticket back on the rail), redelivered to another consumer. This is exactly why duplicates exist under at-least-once.
...
- **Push-based brokers (RabbitMQ)** flood consumers unless you bound them: set a **prefetch / QoS limit** (e.g. `prefetch=10`) so the broker only pushes N un-acked messages per consumer and waits. Forget it and a fast producer + slow consumer = the consumer OOMs (out-of-memorys) on an unbounded in-flight backlog. Push is lower-latency but you **own the flow control**.
