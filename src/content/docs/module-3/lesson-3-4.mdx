...
The cost is quantifiable, and it is the heart of the Director conversation: a `W = R = 1` read hits the nearest replica at **~1-5 ms**; a `W = R = 2` read waits on the 2nd-fastest of 3 replicas (often cross-AZ) at **~5-15 ms**; a cross-region strong read runs to **tens-hundreds of ms** (RTT ~150 ms). Availability moves the opposite way: writes tolerate `N − W` replicas down, reads `N − R`. **Same store, same three replicas, two settings, picked per data-flow.** This is the PACELC dial expressed as numbers.
