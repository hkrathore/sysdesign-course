...
1. **Which desk takes the bag?** A rule maps each ticket number to a desk: the **partitioning** function, the consistent-hashing ring, so opening a new desk doesn't force the hall to re-file every bag.
