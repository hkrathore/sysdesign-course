...
- Carry the delivery and ordering rules forward from the messaging-queue building block (**at-least-once + idempotency**, **ordering per partition/key**) into a fan-out world, and add the two new axes: **push vs pull delivery** and **retention/replay**.
...
- **Point-to-point queue:** a message is delivered to **exactly one** consumer in the pool. N workers **compete** to drain the queue; each message is worked once. This is **work distribution**, scale throughput by adding competing workers.
...
**Delivery guarantees and ordering, carried forward from 3.8, not re-derived.** The messaging-queue building block established the rules and they hold identically here, so the Director move is to **reference** them, not re-explain:
...
The messaging-queue building block ended its checkout example with the tee-up: *if fraud, analytics, and fulfillment all need to read every order, that's pub-sub.* Here it is. The same `OrderPlaced` event must now drive **five independent reactions**, each owned by a different team, and the set will grow.
...
- **Ordering:** per-order events (placed → paid → refunded) must stay ordered, so partition by **`order_id`** (the rule), per-order order for every group, full parallelism across orders.
