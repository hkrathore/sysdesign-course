...
**Nearest-first / SSTF (always serve the closest call).** Best average wait on uniform traffic, **unbounded worst case**. The starvation trace: car at floor 3; caller at floor 28; lobby floors 1-5 generate a call every ~20 s (they do, at lunch). Nearest-first *always* finds a closer call than 28, floor 28 is **formally starved** while the average looks great (the far end of the street, never served). Proposing SSTF without this trace fails the senior bar; proposing it *and naming the starvation case unprompted* is what rubrics reward.
