...
- Reason about the two multipliers that compound the win, **compression** on homogeneous columns (typically **3–10×**, via run-length / dictionary / delta / bit-packing) and **vectorized/SIMD (single instruction, multiple data) execution** (~**10–100×** per core), and tie both back to the scan-cost = bytes-scanned model.
...
- **Believing column stores are universally better.** They're worse at point lookups (rows scattered across chunks) and row-level updates (append-mostly, like an LSM (log-structured merge)), which is exactly why OLTP stays on row stores.
...
*End of Lesson 7.2. You now own the mechanics behind the columnar claim: columnar layout reads only the columns and blocks a query needs, compressed and vectorized, which is why an analytical scan is orders of magnitude cheaper than the same query on a row store.*
