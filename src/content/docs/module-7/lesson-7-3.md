---
title: "7.3 - Data Warehouse, Lake & Lakehouse"
...
> Warehouse, lake, and lakehouse are one evolution, not three products: the warehouse bundles governance with proprietary storage at a premium, the lake dumps open files on ~\$23/TB-month object storage (tiering to \$1–4) and forfeits every guarantee, and the lakehouse adds a thin metadata layer, the open table format, that buys the guarantees back. The whole decision is where you take your ACID: bundled and locked in, absent, or as Iceberg/Delta/Hudi snapshots over Parquet any engine can read. One move to remember: warehouse guarantees, lake prices.
...
- State what an **open table format** (Iceberg / Delta Lake / Hudi) adds over a directory of Parquet files, **ACID (atomicity, consistency, isolation, durability) commits, snapshot isolation, schema & partition evolution, time travel, concurrent writers**, and how it works in one line: a **metadata layer of snapshots and manifests over Parquet**.
...
flowchart TB
    ...
    subgraph LH["3 · Lakehouse (open files + table format)"]
      ...
      OBJ["Object storage S3/GCS · ~$23/TB-mo → ~$1–4 tiered"]
      ENG --> CAT --> TF --> PQ --> OBJ
    end
...
The Director move is choosing on **scale, workload mix, lock-in tolerance, and ops maturity**, warehouse for BI-first and ops-light, lakehouse for PB-scale-plus-ML-plus-openness, and never treating a raw-file lake as the trustworthy store. Note the convergence worth stating: **Snowflake and BigQuery now query external Iceberg tables**, so the managed engine and the open lakehouse are meeting in the middle, a managed query engine pointed at open lakehouse storage, which is the pragmatic end-state.
...
- **Confusing schema-on-read with "no schema ever."** Schema-on-read defers structure to query time for flexibility; it does not mean ungoverned forever. The lakehouse layers schema-on-read raw data into conformed, schema-on-write-grade tables, flexibility *and* trust.
