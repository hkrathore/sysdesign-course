...
> An analytical schema is the production database turned inside out: OLTP normalizes so a thousand writers never contradict each other, analytics denormalizes so one reader can sweep fifty million rows without a join. The whole lesson turns on that inversion, and on declaring the grain of a fact table before naming a single column. Get it right and a five-join, 50M-row query collapses to a ~5K-row pre-aggregate, roughly a 10,000× scan cut; get history wrong (SCD Type 1) and last year's California revenue silently becomes Texas revenue.

### Learning objectives
- State the **inversion**: OLTP (online transaction processing) schemas are normalized for *correct, conflict-free writes on one entity*, analytical schemas are **denormalized and modeled for scans and human questions over all entities**, and the same data is laid out the opposite way for the opposite job.
...
- **SCD Type 1, overwrite.** Just update the attribute in place (the clerk's one-card edit); history is lost. Simple, no row growth. *Use when* the attribute genuinely has no analytical history value (a typo correction, a cosmetic rename). The cost: every historical query silently re-attributes the past to the present, last year's California sales retroactively become Texas sales, and nobody notices.
