...
- Run the **data-as-a-product** operating model: named owners, SLAs/SLOs (service-level objectives), docs, and support for each data product, so trust is engineered rather than assumed (the quality discipline, applied to the org).
...
*End of Lesson 7.13. You can now run the data function, not just build its pipelines, the org shape, the data-as-a-product operating model, the reliability contracts and on-call, and the self-serve access model that makes data a product the business trusts and serves itself.*
