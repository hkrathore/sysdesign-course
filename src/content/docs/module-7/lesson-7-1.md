...
- Reason in the platform's four governing quantities, **freshness, scan-cost, volume, and trust**, the way you reasoned in QPS (queries per second) and storage for OLTP, and know that pushing freshness costs money and complexity.
...
- **Vectorized execution.** OLAP engines process columns in batches through SIMD-friendly (SIMD = single instruction, multiple data) loops rather than a tuple-at-a-time iterator, often 10–100× the per-core throughput on aggregation.
...
- **Why not just index Postgres?** Indexes accelerate selective point/range lookups; an analytical aggregate is *unselective* (it touches most rows), so the planner ignores the index and table-scans anyway, now competing with your OLTP traffic for the same buffer pool and IOPS (I/O operations per second). The fix isn't a better index; it's a different storage paradigm.
