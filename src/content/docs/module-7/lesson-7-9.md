...
- Reason in the numbers that make trust real, freshness SLA (service-level agreement) windows, volume-anomaly thresholds (±X%), the **detection lag** that separates a week-long silent break from a five-minute caught one, and the dollar cost of the wrong decision that lands downstream.
...
**Tests in the pipeline are the regression suite for data, the exact analog of the LLM eval gate.** There, every prompt/model/RAG (retrieval-augmented generation) change must pass a golden set before it ships, because "looked fine in a demo" is how quality silently regresses. Identical structure here: every model run must pass a fixed assertion set before its output is trusted downstream, because "the query ran without error" is how a wrong number silently ships. The tooling:

- **dbt tests**, assertions on your transformation models, declared in YAML beside the model. Built-ins cover `unique`, `not_null`, `accepted_values`, `relationships` (referential integrity); packages (`dbt-utils`, `dbt-expectations`) add ranges, freshness, row-count anomalies. They run *as DAG (directed acyclic graph) nodes*, so a downstream model can be configured not to build on a failed upstream test.
