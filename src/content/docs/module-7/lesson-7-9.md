...
**Network play → ship moves, not boards.** Client-server, **server-authoritative**: clients *propose* moves; the server runs the same `Game` validation and broadcasts accepted moves with a **sequence number** (Lesson 3.6's problem in miniature, both clients must apply in identical order). Never trust client validation, a hacked client is just an HTTP client. A move is ~4 bytes vs 64+ for a board, but the real win is replayability: reconnect = replay from last sequence number; spectators = subscribe to the log (Lesson 3.9's shape); persistence = the append-only log itself. The move log *is* the wire protocol *is* the storage format, one decision in D, three payouts.
...
> *Model:* Client-server, server-authoritative: the server owns the only trusted `Game`; clients send proposed moves (~4 bytes + framing); the server validates with the same two-layer pipeline and broadcasts accepted moves with sequence numbers so both clients apply in identical order, Lesson 3.6's ordering problem in miniature. Never trust client validation: a hacked client is just an HTTP client. Reconnect = replay from the last acked sequence number; spectators subscribe to the same log (Lesson 3.9); persistence is the log itself. The decision that made this a two-minute answer was made in the data model: keep moves, not snapshots.
