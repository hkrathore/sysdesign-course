...
> A data platform is not a pipeline, it is a graph: hundreds of DAGs, tens of thousands of task instances a day, each firing only when its upstream's success is recorded. The orchestrator (Airflow/Dagster) owns sequence, retries, and the 07:00 SLA; dbt owns the transform as versioned, tested SQL run inside the warehouse. The property everything hangs on is idempotent, partition-scoped tasks: with them, a 90-day logic bug is a quotable backfill (90 × 200 GB × \$5/TB ≈ \$90, under an hour); without them, every retry double-counts revenue.
...
- Reason about a pipeline as a **DAG** (directed acyclic graph): tasks, dependency edges, schedules, retries with backoff, and SLAs/alerting, reusing the scheduler/executor machinery of 3.15 and 5.14 rather than re-deriving it, and knowing where the data-platform orchestrator adds *data-aware* concerns the generic scheduler does not.
...
- **HA (high availability) scheduling reuses SKIP LOCKED.** Airflow 2.x runs multiple active schedulers against one metadata DB using `SELECT … FOR UPDATE SKIP LOCKED` (the exact pattern) so two schedulers never claim the same task instance, the decentralized-contention topology, not a single leader.
...
- **"Where does dbt fit, and what does it replace?"**, *Strong:* dbt is the **T** of ELT (extract, load, transform), SQL models compiled and run *in the warehouse*, `ref()`-derived DAG and lineage, built-in tests and docs, incremental models, replacing untested/unversioned scripts or stored procs with versioned, tested, rebuildable transformation (analytics engineering). *Red flag:* thinks dbt ingests data, or that dbt schedules itself.
...
*End of Lesson 7.7. The control plane is what turns a heap of jobs into a reliable, rebuildable platform: an orchestrator that schedules a DAG of idempotent, partition-scoped tasks (so backfill is a costable re-run, not an outage), and dbt doing the versioned, tested "T" inside the warehouse. Next: 13.8, dimensional modeling and the medallion architecture (star schemas, SCDs (slowly changing dimensions), bronze/silver/gold), the *what* those dbt models build, to this lesson's *how* they run.*
