...
> Multi-tenancy is not a `tenant_id` column, it is a business model: one shared system serving thousands of customers who must never see each other's data. Every layer sits somewhere on the silo-bridge-pool spectrum, and one trade recurs at every point: isolation versus efficiency, priced per tenant tier. The numbers: pool the long tail at ~\$5/tenant for ~97 percent infrastructure margin, silo the regulated whale at ~\$4k/tenant because isolation closes the deal. And one failure outranks everything: cross-tenant leakage, worse than downtime because it cannot be un-happened.
...
- **Billing and metering.** **Meter per-tenant consumption** (API calls, storage, compute) into an invoice (the meter on each unit); without it you cannot price by usage or know your cost per tenant.
