...
> "Design a multi-tenant SaaS" is a platform question, not a CRM one: the app is commodity; the machinery that keeps 100,000 tenants isolated, fair, and profitable is the interview. The whole design turns on one call, the tenancy model per tier: pooled shared-schema with RLS packs ~5,000 SMB tenants per shard at ~$0.50 each, while the top 1% (holding ~55% of the 5M seats) buy database-per-tenant silos. Everything else, per-tenant limits, metering, pool-to-silo migration, hangs off that split. One failure outranks downtime: the cross-tenant leak, designed out by construction.
