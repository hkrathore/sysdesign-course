...
> Tenant isolation comes in three strengths, and the gap between them is the business model. Database-per-tenant buys physical separation at a $15 to $30/month floor no free tier survives; schema-per-tenant hits catalog bloat past low thousands; shared-schema with `tenant_id` costs cents per tenant but sits one forgotten `WHERE` clause from leaking every customer at once. The resolution is tiering: pool the long tail, silo the regulated whales. And the pool gets two independent guards, application scoping plus Row-Level Security, so a leak takes two failures, not one.
