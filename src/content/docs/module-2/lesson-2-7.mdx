...
- **PC/EC, Google Spanner, VoltDB, classic single-leader RDBMS (relational database management system) clusters.** Spanner uses Paxos majorities and TrueTime commit-wait: under a partition the minority cannot form a majority and **stops serving** there (**PC**, it chooses consistency over availability); in normal operation it **still** pays the coordination/commit-wait cost to remain externally consistent (**EC**). It does *not* sacrifice consistency for latency, that is the defining PC/EC stance, and the reason Spanner advertises "external consistency" rather than "low latency."
