...
That is the whole of CAP: *when the line is cut, you pick correctness or uptime — you cannot have both.* The deeper, more useful question is what the clerks do on a **normal day when the line works fine**. Do they confirm every change with the other office before telling the customer "done" (slower, but both ledgers always agree)? Or do they answer instantly and sync in the background (faster, but a reader in the other office might briefly see stale numbers)? **That** everyday trade — speed versus agreement when nothing is broken — is what PACELC adds, and it is the one you actually pay for every single day.
...
- **PC/EL — PNUTS (Yahoo).** The subtle quadrant, and Abadi's canonical example — and the one people get backwards. PNUTS gives **per-record timeline consistency** by funneling each record's writes through that record's **master**. On a normal day its default read (`read-any`) returns whatever the **nearest local replica** holds — possibly stale — for minimal latency (**EL**): it gives up consistency for speed when nothing is broken. But under a partition, a region that can no longer reach a record's master simply **cannot accept writes** for those records — it gives up **availability** to avoid violating the per-record timeline (**PC**), *without* relaxing consistency further. So it is the inverse of the eventual-consistency stores: lax on a good day, strict when the network breaks. (Counter-intuitive enough that asserting PNUTS is "PA/EC" is a common error — it is **PC/EL**.)
- **PA/EC — MongoDB with majority concern (the genuinely rare corner).** "Strong-ish normally, but availability wins when the network breaks" is the hard quadrant — Abadi calls a *true* PA/EC system close to mythical, and even MongoDB only lands here under a specific config. Run it with `w:majority` writes and `readConcern:"majority"`/`linearizable` reads and it coordinates a majority on the normal path, paying latency for consistency (**EC**); under a partition the side that retains a primary keeps serving, favoring availability on the majority side (**PA**). The reason this quadrant is rare is that it demands a system engineered to *change its mind* about consistency exactly at the partition boundary — most stores pick one stance and hold it. Knowing PA/EC is the scarce corner (and PC/EL is occupied by PNUTS, not empty) *demonstrates you understand the framework rather than memorized four labels.*
...
- The same company runs **both** in one product (polyglot persistence, Lesson 2.2): Postgres/Spanner for the ledger, DynamoDB/Cassandra for the feed. **The signal is choosing the axis per data-flow and naming the cost — not labeling the whole company "AP."**

This builds directly on replication (Lesson 2.4): leaderless quorum stores are the tunable PA/EL↔EC family, single-leader systems are PC-leaning. It sets up Lesson 2.8, where the **W + R > N** quorum rule is the concrete dial that *moves a system along the PACELC axes* — turning these letters into numbers you can set.
...
The interactive explorer below makes this concrete. Toggle the **partition** switch to flip into the CAP regime, where the only choice is **C or A**; release it to land in the **Else**, where the choice is **Latency or Consistency**. As you move the consistency/latency lever, the panel plots where named systems sit — DynamoDB and Cassandra clustering in **PA/EL**, Spanner and VoltDB in **PC/EC**, PNUTS in the often-misplaced **PC/EL** corner, and majority-concern MongoDB in the rare **PA/EC** corner — and shows, for the configuration you pick, the round-trip cost you take on (e.g., a local `ONE` read at ~1–5 ms versus a cross-AZ `QUORUM` read at ~5–15 ms, or a cross-region strong read in the tens-to-hundreds of ms). The point it drives home: a single store can be **dragged between quadrants** by its quorum settings, so the label is a *consequence of configuration*, not an identity.
...
- **"How would you *move* this system's consistency without changing databases?"** — *Strong:* turn the quorum dial — raise `W`/`R` toward `W+R>N` (Lesson 2.8) to shift **E** from **EL** to **EC**, accepting more latency. *Red flag:* treating the C/A character as immutable per product.
...
> *Model:* CA isn't a runtime option for anything spanning a network — partitions are inevitable (NIC/switch/fiber failures, GC pauses, overloaded links create effective partitions), so P is a given. The real choice under partition is **CP** (stay correct, minority stops) or **AP** (stay available, reconcile later). "CA" only describes a single non-distributed node or the system's behavior while no partition is occurring. I'd ask them to instead specify, per operation, whether correctness or uptime wins when the network breaks — and to name the cost of the side they drop.
...
> *Model:* Mechanically: raise the quorum so **W + R > N** (e.g., `N=3, W=2, R=2`) → reads now overlap the latest write set, moving the store's **E** behavior from **EL** to **EC** (Lesson 2.8). Cost: every read/write now waits on a majority of replicas (often cross-AZ), so latency rises from ~1–5 ms toward ~5–15 ms+ and availability drops (you need 2 of 3 up, not 1 of 3). Director pushback: *does the feed's requirement actually justify it?* Feeds are the canonical AP/EL case — staleness is usually invisible. I'd confirm the real NFR before paying a latency/availability tax on a path that rarely needs it, and likely reserve strong consistency for the ledger, not the feed.

**Q4.** Reconcile: "Spanner is CP" with "Spanner offers five-nines availability." Is that a contradiction?
> *Model:* No. CAP/PACELC's "CP" describes the choice made *in the instant of a partition* — the minority replica set stops serving to avoid divergence. That moment is rare. Yearly availability (five-nines = ~5 min/year of downtime) is an *aggregate over time*, dominated by the 99.99%+ of time with no partition, where Spanner is fully available. Redundant paths and TrueTime keep partitions short and infrequent. So "sacrifice availability during the rare partition" and "extremely available across the year" are entirely compatible — CP is about *which property you drop when forced*, not how often you're down.
...
- Classify by behavior: **PA/EL** (Cassandra/DynamoDB — fast, eventual), **PC/EC** (Spanner/VoltDB — correct, coordinated), **PA/EC** (majority-concern MongoDB — strong-ish normally, available under partition), and the often-misplaced **PC/EL** (PNUTS — strict under partition, fast/stale local reads normally).
- It's a **per-operation requirements decision**: ledger → CP/EC, feed → AP/EL, in *one* product; name the cost of the side you drop, and remember the quorum dial (2.8) slides a store between quadrants.

> **Spaced-repetition recap:** The dead phone line: under a partition pick correctness (CP) or uptime (AP) — there is no CA. PACELC adds the normal day: even with no partition, strong consistency costs round trips (EC) while local reads are fast but stale (EL). Cassandra/DynamoDB = PA/EL, Spanner = PC/EC, majority-MongoDB = PA/EC, PNUTS = PC/EL (strict under partition, fast/stale normally — the corner people invert). Choose per operation, name the cost, turn the quorum dial (2.8) to move along the axis.

---

*End of Lesson 2.7. Next: 2.8 — Consistency models + quorum (W + R > N), where the PACELC axes become concrete N/W/R numbers you can set (+ Quorum calculator).*
