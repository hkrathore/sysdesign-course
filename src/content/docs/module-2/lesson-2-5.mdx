...
> Partitioning starts when one node's ceiling breaks: 50 TB won't fit a 2 TB box, and ~700k writes/s swamps a ~30k writes/s node no matter how many replicas you add. Splitting is easy; the fight is the placement rule, range (cheap scans, hot-shard-of-the-day), hash (even spread, no scans, and `mod N` remaps ~80% of keys when you grow), or directory (surgical placement behind a load-bearing lookup). The real decision is the partition key: it pre-commits which query stays cheap and which you just made expensive.

### Learning objectives
- Explain *why* we partition, when data volume or write throughput outgrows a single node, and distinguish partitioning from replication (the topic of the replication lesson).
...
- Reason about **hot spots / celebrity keys**, know which strategies are vulnerable to them, and how each is mitigated.
...
### Intuition first
Replication was photocopying one book so many people could read it at once. **Partitioning is the opposite problem: the library has grown too big for one building, so you split the *collection* across several branches**, no single branch holds everything. Now every patron needs a rule for *which branch* a given book lives in.
...
**1) Range partitioning.** Contiguous key ranges per partition (`A-F`, `G-M`, `N-Z`; Jan-Mar on shard 1). *Win:* **range scans are cheap and local**, and keys arrive sorted, exactly what an LSM-tree (LSM = log-structured merge) wants. *Cost:* **skew is the default failure mode**, range-partition by timestamp and *all* of today's writes land on the single newest shard (the "hot-shard-of-the-day"). Used by HBase, Bigtable, MongoDB (ranged), CockroachDB, and Spanner; these systems lean on **automatic range splitting**: a too-big or too-hot range splits in two and one half migrates, no human required.

**2) Hash partitioning.** Hash the key and let the hash decide, naively, `partition = hash(key) mod N`. *Win:* uniform scatter; sequential/time-ordered keys stop hot-spotting. *Cost:* **locality destroyed**, range scans become a scatter-gather across every partition (browsing A-F means visiting every branch), and `mod N` has a second, vicious cost: **change N and almost every key remaps** (below). Used by Cassandra and DynamoDB (partition-key hashing), Redis Cluster (hash slots), MongoDB (hashed). The common refinement: **hash a high-cardinality prefix to pick the shard, keep a sortable suffix for local ordering**, Cassandra's compound partition/clustering keys, buying even spread *and* local range scans within a partition.

**3) Directory / lookup partitioning.** An explicit **lookup table** mapping key → partition, consulted on every request. *Win:* **total flexibility**, place any key anywhere, move a single hot key, run heterogeneous shards. *Cost:* an **extra hop on every request and a potential SPOF/bottleneck** (every patron starts at the index desk), replicate and cache it aggressively. The model behind **Vitess** (sharded MySQL at YouTube/Slack) and the **HDFS (Hadoop Distributed File System) NameNode / GFS master**, the directory's availability *is* the system's availability.

**Hot spots / celebrity keys.** Even a perfect hash assumes key *access* is uniform; real workloads are Zipfian, a Justin Bieber tweet or a Black-Friday SKU (stock-keeping unit) concentrates traffic onto **one key**, and one key lives on one partition (one branch, mobbed). **Hashing spreads keys, not a single key's load.** Mitigations, in escalating order:
- **Salting**: append a bucket suffix to spread one hot key across ~10 physical partitions, at the cost of a scatter-gather on read (DynamoDB's "write sharding").
- A **dedicated shard** for known whales (placed via a directory).
- **Cache the hot key** in Redis, the most common real answer for read-hot keys.
- **Fan-out-on-read** for celebrities instead of fan-out-on-write (Twitter's hybrid timeline).

The Director signal: *uniform key spread ≠ uniform load*, and naming the salt/cache/dedicated-shard fix.

**Rebalancing, the `mod N` cliff.** When N changes, reassignment means **copying real bytes over the network while serving live traffic.** Under `hash(key) mod N`, going from 4 to 5 nodes remaps roughly **80% of all keys**, on a 50 TB dataset that's ~40 TB of movement and a cache-miss storm just to add 25% capacity. That pain is precisely what **consistent hashing** exists to fix: only ~**1/N of keys** move when a node joins or leaves. Range systems avoid the cliff by splitting/merging individual ranges; directory systems edit the lookup table for just the moved keys.
...
| **Use when…** | range/recency queries dominate (time-series, leaderboards, scans) | uniform point-lookups, even spread, no scans (KV (key-value), sessions, feeds) | you need surgical placement / heterogeneous shards / per-key moves (Vitess, GFS/HDFS masters) |
...
- **"You add a node to a 4-shard hash cluster, what happens?"**, *Strong:* under `mod N` ~80% of keys relocate (a data-movement and cache-miss storm), which is *why* consistent hashing exists (~1/N moves). *Red flag:* assuming adding a node is cheap/instant.
...
- **Ignoring the `mod N` rebalancing cliff**, naive hash sharding makes adding capacity move almost the entire dataset (the motivation for consistent hashing).
...
**Q4.** When is directory/lookup partitioning worth its extra hop and SPOF (single point of failure) risk over plain hashing?
...
- **`hash mod N` rebalancing is catastrophic**, ~80% of keys move to grow a 4-node cluster by one; exactly what consistent hashing fixes (~1/N moves).
