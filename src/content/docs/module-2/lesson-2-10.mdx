...
- **Arbitrary query complexity is an availability and cost risk.** A deeply nested query can melt your DB, so you need depth/complexity limits, and you've moved real engineering effort server-side.
...
- **Stateless (externalized session):** move the session into a **shared, fast store**, **Redis** (the key-value family): a session token keys a small blob, sub-millisecond reads, TTL (time-to-live) expiry built in. Now **any** node serves **any** request; the LB runs plain round-robin, load spreads evenly, **adding node 11 instantly takes its share**, a dead node takes zero state with it, deploys are trivial. You scale by **adding identical, disposable nodes**, the definition of horizontal scale.
