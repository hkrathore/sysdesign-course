...
- **Stateless (externalized session):** move the session into a **shared, fast store** - **Redis** (2.2's key-value family): a session token keys a small blob, sub-millisecond reads, TTL expiry built in. Now **any** node serves **any** request; the LB runs plain round-robin (Lesson 3.2), load spreads evenly, **adding node 11 instantly takes its share**, a dead node takes zero state with it, deploys are trivial. You scale by **adding identical, disposable nodes** - the definition of horizontal scale.
...
But a cache is a **second copy of the truth**, so you must answer: *when the data changes, how do cache and database stay in agreement, and who pays the latency?* This is 2.7's consistency-vs-latency tension made concrete at the cache layer. The question has three classic answers on the **write** path, trading **consistency, write latency, and durability** against each other - which is precisely what the simulator below lets you feel.
...
- **App tier:** every node is **stateless** behind an L7 load balancer (2.1); session/auth lives in **Redis** keyed by token, so any node serves any user and we scale by adding identical nodes. Sticky sessions are explicitly rejected - they'd pin hot users and make deploys drop sessions.
