...
For replication, you don't stop at the first node, you keep walking clockwise and place the next `RF−1` (replication factor minus one) distinct physical nodes as replicas. Cassandra's `NetworkTopologyStrategy` does exactly this, skipping nodes to land replicas in different racks/AZs. The ring isn't just a partitioner; it's also the **replica-placement** function.
