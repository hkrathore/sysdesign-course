...
> Replication is deciding who may hold the pen: one scribe (leader-follower), a scribe per region (multi-leader), or no scribe at all, just a majority vote (leaderless, W + R > N). Every topology buys availability, read scale, latency, or durability by taking on a named cost: replication lag and failover risk, write conflicts, or read-repair complexity. The lesson's real subject is that cost: async followers serve stale reads, failover can lose writes or split-brain, and "last write wins" silently drops data.
...
- **Replication-lag anomalies and their fixes:** *read-your-writes* (you post, then read a lagging follower and don't see your post → route a user's reads to the leader or a known-current replica for a window); *monotonic reads* (time appears to go backwards across reads → pin a user to one replica); *consistent prefix* (causally-ordered writes appear out of order → route causally-related writes through the same partition/replica).
- **Failover is the hard part:** detect leader death (timeout tuning, too tight = false failovers, too loose = long outages), promote the **most up-to-date** follower (the best-informed assistant takes the pen), and prevent **split-brain** (two nodes both believing they're leader → fence with epochs/leases/quorum). Async failover can **lose** the old leader's unreplicated writes.

![Two timelines showing when the client receives its acknowledgement: synchronous replication returns OK only after the follower acks, so write latency includes the follower round trip; asynchronous returns OK immediately and replication continues in the background, opening a lag window of stale reads and writes lost on leader death](../../../assets/images/module-2/replication-sync-vs-async.png)

*The ack point is the whole trade: synchronous waits out the follower round trip; asynchronous returns immediately and opens the lag window where stale reads and lost writes live.*

**Multi-leader**, several leaders accept writes (often one per region). Pro: low write latency in every region and survival of a region partition. Con: **write conflicts** when the same datum is edited in two leaders (two scribes, one line). Resolution strategies, weakest→strongest: **last-write-wins** by timestamp (simple but silently drops data), **version vectors** (detect concurrent edits), **CRDTs** (conflict-free replicated data types) (conflict-free merge for suitable data types), or **application-level merge**. Used in multi-DC databases, calendar/contact sync, and collaborative editing.

**Leaderless (Dynamo-style)**, no leader; a client or coordinator writes to **N** replicas and waits for **W** acks, and reads from **R** replicas, taking the newest (the majority view of the truth). The **quorum rule W + R > N** guarantees a read set overlaps the latest write set. Node failure needs no failover, you just talk to whoever's up, and convergence is maintained by **read repair** (fix stale replicas on read), **anti-entropy** (background replica comparison and sync), and **hinted handoff** (a neighbor holds a down node's writes and replays them when it recovers). Used by Cassandra, DynamoDB, Riak. Offers **tunable consistency** by choosing N/W/R.
...
- **Globally-accepted writes that must survive a region cut** (e.g., the shopping cart that motivated Dynamo): **leaderless quorum**, AP-leaning (stays available under partition, at consistency's cost), with conflict resolution (version vectors / merge, Amazon famously let a re-added deleted item win to avoid losing a sale). You accept conflict-handling complexity to get always-writable, partition-tolerant behavior.
