...
| **DNS-based / GSLB** (global server load balancing) | Global, simple, no single choke point | Slow failover (TTL caching), coarse | Geo steering across regions |
...
- Treating "the load balancer" as one box, it's itself a redundancy concern and a potential SPOF (single point of failure).
