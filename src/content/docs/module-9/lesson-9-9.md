...
> An agent is nothing more than an LLM in a loop where the model, not your code, chooses the next action: an improvising cook instead of a recipe card. That autonomy buys tasks you can't pre-script and costs reliability by construction: at 95% per step, a 10-step run succeeds only ~60% of the time, a 20-step run ~36%. The decision the whole lesson turns on is agent vs workflow, and the senior answer is the most constrained option that works; most "agents" should be workflows.
