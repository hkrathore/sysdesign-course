...
- **Critical metadata:** every chunk carries the **ACL** (access control list) of its source doc. This is non-negotiable and seeds the scariest failure mode.
...
- **"How do you keep the index fresh?"** — *Strong:* incremental, event-driven upserts on source change with delete tombstones; states the freshness SLA (service-level agreement) as the ingest-pipeline latency. *Red flag:* "re-index nightly" with no awareness of the staleness window or the re-embed cost at scale.
