...
- Treat memory as a **cost + quality + privacy** decision: session vs cross-session, multi-tenant isolation, PII (personally identifiable information) governance and deletion. Default to **retrieval + summarization over a bigger context window.**
...
| **Semantic** | Durable facts about the user/domain | "User is a backend lead, prefers Postgres, company is in the EU (GDPR (General Data Protection Regulation))" | **Structured DB / profile** (key-value or rows) |
...
- **Vector store + retrieval** for episodic memory: embed each past interaction; on a new turn, embed the current request and pull the top-k most similar past events into context. This is **RAG (retrieval-augmented generation) over interactions** — same machinery, same failure modes (embedding quality, chunking, the retriever missing a relevant memory).
