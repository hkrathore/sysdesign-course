...
> A chatbot is an advisor; an agent is an **operator** with your database, your credit card, and the send button. The question stops being "is the output good?" and becomes **"what's the blast radius when it's wrong?"**: a 95%-correct model wired to a destructive tool fires the wrong action one time in twenty. Everything turns on setting autonomy per action, by **reversibility and impact**, never by model capability. And because injection-to-action cannot be prevented, the design is containment: least privilege, sandboxes, deterministic guardrails, and a human on the irreversible.
