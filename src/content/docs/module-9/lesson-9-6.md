...
- Assemble a **defense-in-depth** stack — input/output classifiers, system-prompt hardening + instruction hierarchy, structured output + schema validation, allow-lists, PII (personally identifiable information) redaction, grounding/citations, output encoding/sandboxing, rate-limiting — and place the **trust boundary outside the model**.
...
- **PII detection / redaction.** Redact PII on the way *in* (so it never reaches the provider or your logs) and *out* (so it never reaches the wrong user) — Presidio or provider DLP (data loss prevention), tied to your retention/governance policy.
...
- **Retrieval ACLs (access control lists):** the poisoned chunk should be tagged and filtered so it's only retrievable in its own ticket's context — narrowing *which* queries can ever see it. Rejected alternative: a flat, un-permissioned index, the most common way poison spreads to every user.
...
- **"User data goes into the prompt — what governance applies?"** — *Strong:* names the data-flow question (PII goes to a third-party provider and into logs/traces, possibly retained/trained-on), and the controls — PII redaction in/out, retention policy, self-host or BAA (business associate agreement) when required. *Red flag:* treats the prompt as a private internal buffer.
