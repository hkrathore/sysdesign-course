...
> Vector search is a **library shelved by meaning, not title**: exact nearest-neighbor search is `O(N·d)` per query, fine at 100k vectors and hopeless at 10M, so you trade a few points of **recall** (target 95–98%) for large speedups. The whole design is one triangle, **recall vs latency vs memory, pick two**, and the index (flat, IVF, HNSW, HNSW+PQ) is how you pick. The Director layer on top: reuse pgvector or OpenSearch before buying a vector database, and apply ACL filters **before** the search, never after.
...
- Frame the **approximate-nearest-neighbor (ANN)** problem: exact kNN (k-nearest neighbors) is `O(N·d)` per query — fine at 100k vectors, hopeless at 10M–1B — so you trade a little **recall** for large speedups.
- Choose an index — **flat / IVF (inverted file) / HNSW (Hierarchical Navigable Small World) / HNSW+PQ** — from the **recall ↔ latency ↔ memory** triangle, not by reflex, and name what each one costs.
...
- **Flat / brute-force:** no index — compare the query against all `N` vectors and take the top-k (reading every spine in the building). **Recall is 100%** (it *is* the exact answer), it needs **only the raw vectors** in memory, and it's trivial to build (there's nothing to build). The cost is **latency that grows linearly with `N`**. Correct choice **only when `N` is small** — up to ~100k–1M vectors, or as the ground-truth baseline you measure other indexes' recall against. Rejected at scale because per-query latency becomes seconds.
...
- **HNSW + PQ (or scalar quantization):** **Product Quantization** compresses each vector by splitting it into sub-vectors and replacing each with the ID of its nearest centroid from a small learned codebook (the compressed mini-summary on each spine) — a 768-dim float32 vector (3072 bytes) can shrink to **~96–192 bytes, a 16–32× reduction**. **Scalar quantization** is the simpler cousin: store each dimension as int8 instead of float32 (a flat **4× reduction**, ~1% recall loss, far less tuning). You pay a **small recall loss** (PQ approximates distances from the compressed codes) and a **codebook training step**, in exchange for **fitting an index in a fraction of the RAM**. The combo — HNSW for the graph, PQ for compressed vectors — is the standard recipe for **billion-scale** indexes.
...
- **Post-filter:** run ANN for top-k, *then* drop the rows that fail the filter. Simple, but if the filter is selective it **decimates your results** — ask for top-10, get 10 neighbors, 8 fail the ACL (access control list) check, you return **2**. You can't fix this by fetching more without guessing how much more.
...
> **Spaced-repetition recap:** Embeddings = meaning as points in space; same model for corpus and query, or it breaks silently. Exact kNN is `O(N·d)` and too slow at scale → ANN trades recall (95–98%) for speed. The triangle is recall ↔ latency ↔ memory — flat (exact/slow), IVF (clusters), **HNSW (default, costs RAM)**, +**PQ** (compress for billions, small recall loss). Reuse pgvector/OpenSearch before buying a vector DB; **pre-filter** ACLs; go **hybrid** (dense + BM25 via RRF) for exact tokens. Feeds RAG (retrieval-augmented generation) and enterprise RAG; shard the index with consistent hashing.
