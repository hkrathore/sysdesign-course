...
> Adapting a model is a **cost ladder**: prompt, then RAG, then a LoRA fine-tune, then full fine-tune, then continued pretraining, and you climb a rung only when **eval proves the rung below can't hit the bar**. The rule that scores: fresh or private **facts go to RAG**, new **behavior or format goes to a fine-tune**, and the two compose. The trap this lesson exists to kill: **fine-tuning teaches behavior, not facts**, and it welds you to a base-model version whose vendor ships a better one every few months.
...
- **Continued pretraining** — keep training the base model on a large new corpus (billions of tokens) to shift its fundamental knowledge or adapt it to a new domain/language. This is a **platform-team / model-vendor activity**, measured in large GPU clusters and weeks. For an application team, this is **almost never the answer** (you hire someone already schooled) — name it to show you know the ceiling exists, then reject it: the data scale, cost, and expertise dwarf any app-level problem, and RAG + a light fine-tune get you there far cheaper.
