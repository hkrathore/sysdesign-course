...
**The GPU is the unit of cost; HBM is the binding resource.** Unlike a web tier where you reason about disk I/O, CPU, and network, an LLM serving fleet is priced in **GPU-hours** (≈ $2–4/GPU-hour for an H100-class card in 2026), and the thing that runs out first is **HBM** — the 80 GB or so of fast memory soldered next to the chip. Two consumers compete for it: the **weights** and the **KV cache**. The weights alone often nearly fill the card, and what's left bounds your batch — and your batch bounds your throughput. So the first sizing question is never "how many FLOPs (floating-point operations)" — it's "**does the model even fit, and how much room is left for the cache.**"
