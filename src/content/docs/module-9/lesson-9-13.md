...
> A production agent is a **long-running, stateful, failure-prone distributed workflow**, not a chat loop, and it inherits the obligations you already solved in payments and job scheduling. The design turns on one identity: **exactly-once effect = at-least-once execution + an idempotency key**, so a crash between "approve" and "pay" never pays twice. Add durable checkpoints (resume at step 8 of 12, not restart), a human gate on the irreversible, and hard per-task budgets with a kill switch. The reliability story is the design; the prompt is the easy part.
...
- Make side-effecting **tool calls idempotent** by reusing the idempotency-key machinery from the business-domain HLDs (high-level designs), so at-least-once execution never double-acts.
...
**Durable execution: checkpoint state per step; resume, don't restart.** The cardinal sin is keeping agent state in **process RAM**, because a crash, an OOM (out-of-memory), or a deploy then vaporizes a half-finished run. Durable-execution engines persist the outcome of each step to a state store and make the run **resumable**: on restart, completed steps are replayed from the journal (or skipped), and execution continues from the last checkpoint (the flight resumes mid-route, not from the gate). The state — history, current step, pending tool calls — **lives in a durable store, not RAM**. The options span a spectrum:
