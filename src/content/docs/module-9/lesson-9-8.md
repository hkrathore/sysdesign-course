...
> Every LLM call is a taxi with the meter running: each token in and out drops a coin, and the bill scales linearly with usage, so a feature losing $0.30 per user per month only loses more as it grows. Output tokens are priced 3–5× input, and re-sent context taxes every single call. The discipline the lesson drills: pick the lever (routing 5–10×, prompt caching up to ~90% off the prefix, batch ~50% off) and name the quality, freshness, or latency it trades away.
...
**Model routing and cascades — usually the biggest single win.** Most traffic is easy; a minority is hard. A **router** sends each request to the cheapest model that can handle it (the cheap car for the short trip): a small/cheap model by default, **escalating to a larger model only on low confidence or high complexity.** The escalation signal is either a cheap upfront **classifier** (length, topic, difficulty heuristic) or **self-eval** (the small model answers; a quick check — confidence score, a validator, a "can you answer this confidently?" gate — decides whether to retry on the big model). When the cheap model handles the majority of traffic, the blended cost drops **5–10×**, because the expensive model now runs on the 10–20% of requests that actually need it rather than 100%.
