...
export default function EstimationCalculator() {
  ...
  const cardStyle = "rounded-lg border border-slate-700/60 bg-slate-800/40 p-4";
  const accent = "#2dd4a7";
    ...
    <div className="w-full max-w-3xl mx-auto font-mono text-slate-200" style={{ background: "transparent" }}>
      <div className="rounded-xl border border-slate-700 bg-slate-900/80 p-5 shadow-2xl">
        <div className="flex items-center gap-2 mb-1">
          ...
          <h2 className="text-lg font-bold tracking-tight text-white">Back-of-the-Envelope Estimation</h2>
        </div>
        <p className="text-xs text-slate-400 mb-4">Goal: order of magnitude, not accounting. Change one input, predict the output, then check.</p>
          ...
          {FIELDS.map((f) => (
            <div key={f.key}>
              <div className="flex justify-between text-xs mb-1">
                ...
                <span className="text-slate-100 font-semibold">{f.fmt(s[f.key])}</span>
              </div>
              <input
                type="range"
        ...
        <p className="text-[11px] text-slate-500 mt-5 leading-relaxed">
          The interview point is the <span className="text-slate-300">conclusion</span> the numbers justify — e.g. "reads dominate ~{s.rw}×, so the architecture lives on the cache/read path," or "media egress, not QPS, is the binding cost." Round aggressively; ≤3 minutes.
        </p>
      </div>
    </div>
  );
...
function Out({ icon, label, value, formula, accent, highlight }) {
  return (
    <div
      className={`rounded-lg border p-3 ${highlight ? "border-amber-500/50 bg-amber-500/5" : "border-slate-700/60 bg-slate-800/40"}`}
    >
      ...
      <div className="text-[10px] text-slate-500 mt-0.5">{formula}</div>
    </div>
  );
}
