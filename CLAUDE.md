...
- **First-use abbreviation rule (July 2026 glossary pass):** every lesson expands each domain abbreviation at its first prose use — "CRDTs (conflict-free replicated data types)" — because lessons are self-contained and read in any order; never assume an earlier chapter defined it. Universal terms a Director reads fluently (API, HTTP, SQL, CPU, CDN, LLM, AWS product names) are exempt. The course-wide `src/content/docs/glossary.md` (registered in the sidebar) is the canonical reference — add new terms there when a lesson introduces them. Cheat sheets stay telegraphic (no expansions).
...
> **July 2026 — Lesson 5.15 (Review Keyword Search):** added the "search 100M restaurant reviews by keyword" problem to Module 5, whose tail is the build-the-primitive-yourself set (5.13 Kafka, 5.14 KV deep dive). Deliberately distinct from the 3.12 Distributed Search building block, which covers the same inverted index when you *buy* it: 5.15 builds it. The lesson is the course's **arithmetic-before-architecture** problem (1B postings ≈ 1 GB compressed → one machine + 3 replicas, not a cluster), turning on two requirement reads: **boolean AND with no ranking** and **read-only** (which deletes segments/refresh/merges and the consistency model entirely), plus **query-time typo correction** against the 130k dictionary. Cheat sheet updated to 15 cruxes (crux row, numbers row, binding-constraint row, recap). Build **205 pages**.

> **July 2026 — Formats & drills pass (4.19, 17.17, 17.18, 17.14+):** research-grounded follow-up to the 17.16 bank, four additions in one pass. **Lesson 4.19 — The Opening Bank**: 50 rapid-fire design prompts (none duplicating the ~50 existing walkthroughs; curated from Exponent/Hello Interview/interviewing.io/Pragmatic Engineer 2024–26 data incl. the AI-infra and platform-infra rising clusters), each with a collapsed five-beat 90-second RESHADED opening (scope cut → numbers → the turn → shape → depth+delegation); authored by 4 concurrent subagents from a per-prompt spec. **Lesson 17.17 — The Written Round**: the five doc/presentation formats (Stripe one-pager+20-min, Amazon 2-page narrative, document-of-significance, 30-60-90, strategy case) with answer-first craft and the Q&A defense; 2026 calibration = AI drafting assumed, live defense carries the score. **Lesson 17.18 — Reverse Interview & Offer**: diligence question sets by target + the alignment test, startup capital-structure block, leveling (Director≈L8/D1, $450–650k/yr down-level gap, level→comp→start→paperwork), references-run-on-you, close-on-scope-and-terms. **17.14 deepened**: Microsoft added as sixth archetype (Model-Coach-Care, growth mindset, As-Appropriate closer), Meta feedback-seeking scored by name, out-of-org values-round mechanic. Module 17 = eighteen lessons; course ~860k words; build **204 pages**.

> **July 2026 — Leadership question bank (17.16):** added **Lesson 17.16 — The Question Bank** to Module 17: 60 real Director/VP questions compiled via web research from 2024–26 big-tech loops (Amazon LP rounds, Google org-design/manager-of-managers, Netflix, Meta, Stripe role-plays) and startup/founder loops (First Round, cummulative.io, VC hiring guides) plus 2025–26 era reports (LeadDev, DORA, Jellyfish, Karat), deduplicated against the ~80 questions 17.4–17.14 already treat. Eight sections (founder loop, cross-functional seams, flattening-era org shape, strategy/corp-dev, recruiting machine, AI accountability, team health, self-awareness mirror); every entry tagged shape → framework (STAR-L/CPODT/PMNL/SCQA) with a collapsed 2026-calibrated model answer (active recall); drill protocol, big-tech-vs-startup calibration table, ProbeSimulator re-embedded. Authored by 4 concurrent subagents from a per-section spec against the house rules, then verified (0 em-dashes, 60 balanced details blocks, first-use expansions, build). Module 17 is now sixteen lessons; site builds at **201 pages**.

> **July 2026 — Glossary & first-use abbreviation pass:** a deterministic sweep inserted first-prose-use expansions for ~150 curated domain abbreviations across 174 lesson files (978 insertions; cheat sheets and nav pages exempt), and added the course-wide `glossary.md` (~160 terms, registered in the sidebar after Cheat Sheets). The §7 first-use rule now governs all future lessons.

> **July 2026 — Client & Mobile + Multi-Tenancy/SaaS pass (current live structure; supersedes the June note's numbering below):** added **Lesson 3.18 Microservices Principles** to Module 3, then **renumbered old modules 11–16 → 13–18** (surgical self-identity renumber via anchored patterns — dirs, filenames, `title` prefixes, "End of Lesson" footers, cheat sheets, sidebar, course-home nav; data numbers like burn-rate "14.4×" and pre-existing stale cross-refs left untouched) to open two Part III slots, and authored two new **Specialized Design Tracks** (12 lessons; Opus background subagents from per-lesson specs against the §5 contract; diagrams-only; then verified for 0 em-dashes, structure, and build):
> - **Module 11 — Client & Mobile System Design** (11.1–11.7): client-side-architecture framing (thin-vs-thick, the five client-only constraints), offline-first/local-first sync, conflict resolution (LWW/OT/CRDT/server-authoritative), mobile push & real-time delivery at scale, client performance & resource budgets, web/frontend architecture (rendering/Core Web Vitals/BFF/micro-frontends), + an offline-first mobile RESHADED design problem (11.7).
> - **Module 12 — Multi-Tenancy & SaaS Platform Architecture** (12.1–12.5): tenancy framing (silo/bridge/pool), tenant isolation models (DB-per-tenant/schema/shared+RLS; cross-tenant leakage as the cardinal failure), per-tenant limits/metering/billing, control-plane/data-plane & tenant lifecycle, + a multi-tenant SaaS RESHADED design problem (12.5).
> - Cheat sheets for Modules 11 & 12 added (decision-reference format). Course home refreshed: **~820k words across 18 modules**, new Client/Mobile + Multi-Tenancy/SaaS Fast-Path sections and a landing track card.
>
> **Current Parts (post-renumber, 18 modules):** I · Method & Fundamentals (1–3) · II · Design Problems (4–5) · III · Specialized Design Tracks (6 LLD, 7–8 Data, 9–10 GenAI, **11 Client & Mobile, 12 Multi-Tenancy & SaaS**) · IV · Engineering Excellence & Operations (13 Security, 14 Testing, 15 Troubleshooting) · V · Strategy & Leadership (16 Architecture, 17 Leadership) · VI · Capstone (18). Build green: **197 pages**, node@22.
>
> **June 2026 reorder & de-numbering pass (historical — superseded on numbering by the July pass above):** the course was regrouped into **6 Parts / 16 modules** in learning order, and **all plain-text inter-lesson cross-references were removed** (a deterministic script stripped `Module N` / `Lesson N.M` / `(N.M)` / possessives / forward-nav tails / mermaid-label refs; inline subject-position refs were rephrased to topic names). Lessons are now self-contained, so renumbering is cheap and link-safe. Build green: 158 pages, node@22.
>
